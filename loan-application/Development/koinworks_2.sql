SET @end_period = DATE_FORMAT(NOW(), '%Y-%m-01');
SET @period_1 = DATE_FORMAT(DATE_SUB(@end_period, INTERVAL 1 MONTH), '%Y-%m-01');
SET @period_2 = DATE_FORMAT(DATE_SUB(@end_period, INTERVAL 2 MONTH), '%Y-%m-01');
SET @period_3 = DATE_FORMAT(DATE_SUB(@end_period, INTERVAL 3 MONTH), '%Y-%m-01');
SET @period_4 = DATE_FORMAT(DATE_SUB(@end_period, INTERVAL 4 MONTH), '%Y-%m-01');
SET @period_5 = DATE_FORMAT(DATE_SUB(@end_period, INTERVAL 5 MONTH), '%Y-%m-01');
SET @period_6 = DATE_FORMAT(DATE_SUB(@end_period, INTERVAL 6 MONTH), '%Y-%m-01');
SET @period_7 = DATE_FORMAT(DATE_SUB(@end_period, INTERVAL 7 MONTH), '%Y-%m-01');
SET @period_8 = DATE_FORMAT(DATE_SUB(@end_period, INTERVAL 8 MONTH), '%Y-%m-01');
SET @period_9 = DATE_FORMAT(DATE_SUB(@end_period, INTERVAL 9 MONTH), '%Y-%m-01');
SET @period_10 = DATE_FORMAT(DATE_SUB(@end_period, INTERVAL 10 MONTH), '%Y-%m-01');
SET @period_11 = DATE_FORMAT(DATE_SUB(@end_period, INTERVAL 11 MONTH), '%Y-%m-01');
SET @period_12 = DATE_FORMAT(DATE_SUB(@end_period, INTERVAL 12 MONTH), '%Y-%m-01');
...
FROM
    (SELECT 
        result.name 'shop_name',
            ...
            SUM(IF(tr.created_at >= @period_12
                AND tr.created_at < @period_11, IF(tr.fk_transaction_type IN (13 , 14), tr.value, 0), 0)) 'net_sales_12',
            SUM(IF(tr.created_at >= @period_11
                AND tr.created_at < @period_10, IF(tr.fk_transaction_type IN (13 , 14), tr.value, 0), 0)) 'net_sales_11',
            SUM(IF(tr.created_at >= @period_10
                AND tr.created_at < @period_9, IF(tr.fk_transaction_type IN (13 , 14), tr.value, 0), 0)) 'net_sales_10',
            SUM(IF(tr.created_at >= @period_9
                AND tr.created_at < @period_8, IF(tr.fk_transaction_type IN (13 , 14), tr.value, 0), 0)) 'net_sales_09',
            SUM(IF(tr.created_at >= @period_8
                AND tr.created_at < @period_7, IF(tr.fk_transaction_type IN (13 , 14), tr.value, 0), 0)) 'net_sales_08',
            SUM(IF(tr.created_at >= @period_7
                AND tr.created_at < @period_6, IF(tr.fk_transaction_type IN (13 , 14), tr.value, 0), 0)) 'net_sales_07',
            SUM(IF(tr.created_at >= @period_6
                AND tr.created_at < @period_5, IF(tr.fk_transaction_type IN (13 , 14), tr.value, 0), 0)) 'net_sales_06',
            SUM(IF(tr.created_at >= @period_5
                AND tr.created_at < @period_4, IF(tr.fk_transaction_type IN (13 , 14), tr.value, 0), 0)) 'net_sales_05',
            SUM(IF(tr.created_at >= @period_4
                AND tr.created_at < @period_3, IF(tr.fk_transaction_type IN (13 , 14), tr.value, 0), 0)) 'net_sales_04',
            SUM(IF(tr.created_at >= @period_3
                AND tr.created_at < @period_2, IF(tr.fk_transaction_type IN (13 , 14), tr.value, 0), 0)) 'net_sales_03',
            SUM(IF(tr.created_at >= @period_2
                AND tr.created_at < @period_1, IF(tr.fk_transaction_type IN (13 , 14), tr.value, 0), 0)) 'net_sales_02',
            SUM(IF(tr.created_at >= @period_1
                AND tr.created_at < @end_period, IF(tr.fk_transaction_type IN (13 , 14), tr.value, 0), 0)) 'net_sales_01',
    ...
    FROM
        ...
        sup.id_supplier,
            ...
            (SELECT 
                    COUNT(id_catalog_source)
                FROM
                    bob_live.catalog_source
                WHERE
                    fk_supplier = sup.id_supplier) 'number_of_sku'
    FROM
    ...
    WHERE
        sup.name IN ('Gold Rush Shop' , 'MEDIATECH STORE', 'Mainankayu', 'Bursa BB', 'Channel B')
