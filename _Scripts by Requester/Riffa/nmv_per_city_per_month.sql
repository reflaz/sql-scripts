...
FROM
    ...
    FROM
        ...
        cty.name 'city',
            ...
            reg.name 'province',
            SUM(IF(soish.created_at >= '2016-01-01'
                AND soish.created_at < '2016-02-01', soi.paid_price, 0)) 'january',
            SUM(IF(soish.created_at >= '2016-02-01'
                AND soish.created_at < '2016-03-01', soi.paid_price, 0)) 'february',
            SUM(IF(soish.created_at >= '2016-03-01'
                AND soish.created_at < '2016-04-01', soi.paid_price, 0)) 'march',
            SUM(IF(soish.created_at >= '2016-04-01'
                AND soish.created_at < '2016-05-01', soi.paid_price, 0)) 'april',
            SUM(IF(soish.created_at >= '2016-05-01'
                AND soish.created_at < '2016-06-01', soi.paid_price, 0)) 'may',
            SUM(IF(soish.created_at >= '2016-06-01'
                AND soish.created_at < '2016-07-01', soi.paid_price, 0)) 'june',
            SUM(IF(soish.created_at >= '2016-07-01'
                AND soish.created_at < '2016-08-01', soi.paid_price, 0)) 'july',
            SUM(IF(soish.created_at >= '2016-08-01'
                AND soish.created_at < '2016-09-01', soi.paid_price, 0)) 'august'
    FROM
        (SELECT 
        fk_package
    FROM
    ...
    GROUP BY cty.id_customer_address_region
    HAVING reg.name IS NOT NULL) result) result;