...
FROM
    (SELECT 
        city,
            ...
            rounded_weight,
            CASE
                WHEN FIND_IN_SET(fk_customer_address_region, @kab_gianyar) AND formula_weight >= 1 THEN 'Equal or higher than threshold'
				WHEN FIND_IN_SET(fk_customer_address_region, @kab_gunung_kidul) AND formula_weight >= 1 THEN 'Equal or higher than threshold'
				WHEN FIND_IN_SET(fk_customer_address_region, @kab_tasikmalaya) AND formula_weight >= 1 THEN 'Equal or higher than threshold'
				WHEN FIND_IN_SET(fk_customer_address_region, @kab_jember) AND formula_weight >= 2 THEN 'Equal or higher than threshold'
				WHEN FIND_IN_SET(fk_customer_address_region, @kab_mojokerto) AND formula_weight >= 2 THEN 'Equal or higher than threshold'
				WHEN FIND_IN_SET(fk_customer_address_region, @kota_kediri) AND formula_weight >= 2 THEN 'Equal or higher than threshold'
				WHEN FIND_IN_SET(fk_customer_address_region, @kota_madiun) AND formula_weight >= 1 THEN 'Equal or higher than threshold'
				WHEN FIND_IN_SET(fk_customer_address_region, @kota_mojokerto) AND formula_weight >= 1 THEN 'Equal or higher than threshold'
				WHEN FIND_IN_SET(fk_customer_address_region, @kota_pasuruan) AND formula_weight >= 1 THEN 'Equal or higher than threshold'
				WHEN FIND_IN_SET(fk_customer_address_region, @kota_probolinggo) AND formula_weight >= 1 THEN 'Equal or higher than threshold'
				WHEN FIND_IN_SET(fk_customer_address_region, @kab_lampung_selatan) AND formula_weight >= 1 THEN 'Equal or higher than threshold'
				WHEN FIND_IN_SET(fk_customer_address_region, @kab_gowa) AND formula_weight >= 1 THEN 'Equal or higher than threshold'
				WHEN FIND_IN_SET(fk_customer_address_region, @kab_maros) AND formula_weight >= 1 THEN 'Equal or higher than threshold'
				WHEN FIND_IN_SET(fk_customer_address_region, @kab_bantul) AND formula_weight >= 2 THEN 'Equal or higher than threshold'
				WHEN FIND_IN_SET(fk_customer_address_region, @kab_kulon_progo) AND formula_weight >= 1 THEN 'Equal or higher than threshold'
				WHEN FIND_IN_SET(fk_customer_address_region, @kota_cirebon) AND formula_weight >= 2 THEN 'Equal or higher than threshold'
				WHEN FIND_IN_SET(fk_customer_address_region, @kota_sukabumi) AND formula_weight >= 2 THEN 'Equal or higher than threshold'
				WHEN FIND_IN_SET(fk_customer_address_region, @kota_tasikmalaya) AND formula_weight >= 2 THEN 'Equal or higher than threshold'
				WHEN FIND_IN_SET(fk_customer_address_region, @kota_pekanbaru) AND formula_weight >= 1 THEN 'Equal or higher than threshold'
				WHEN FIND_IN_SET(fk_customer_address_region, @kota_padang) AND formula_weight >= 1 THEN 'Equal or higher than threshold'
				WHEN FIND_IN_SET(fk_customer_address_region, @kota_palembang) AND formula_weight >= 1 THEN 'Equal or higher than threshold'
				WHEN FIND_IN_SET(fk_customer_address_region, @kota_makassar) AND formula_weight >= 1 THEN 'Equal or higher than threshold'
				WHEN FIND_IN_SET(fk_customer_address_region, @kab_badung) AND formula_weight >= 1 THEN 'Equal or higher than threshold'
				WHEN FIND_IN_SET(fk_customer_address_region, @kota_denpasar) AND formula_weight >= 1 THEN 'Equal or higher than threshold'
				WHEN FIND_IN_SET(fk_customer_address_region, @kab_lebak) AND formula_weight >= 2 THEN 'Equal or higher than threshold'
				WHEN FIND_IN_SET(fk_customer_address_region, @kab_pandeglang) AND formula_weight >= 2 THEN 'Equal or higher than threshold'
				WHEN FIND_IN_SET(fk_customer_address_region, @kab_serang) AND formula_weight >= 2 THEN 'Equal or higher than threshold'
				WHEN FIND_IN_SET(fk_customer_address_region, @kota_cilegon) AND formula_weight >= 2 THEN 'Equal or higher than threshold'
				WHEN FIND_IN_SET(fk_customer_address_region, @kota_serang) AND formula_weight >= 2 THEN 'Equal or higher than threshold'
				WHEN FIND_IN_SET(fk_customer_address_region, @kab_sleman) AND formula_weight >= 2 THEN 'Equal or higher than threshold'
				WHEN FIND_IN_SET(fk_customer_address_region, @kota_yogyakarta) AND formula_weight >= 2 THEN 'Equal or higher than threshold'
				WHEN FIND_IN_SET(fk_customer_address_region, @kab_bandung) AND formula_weight >= 2 THEN 'Equal or higher than threshold'
				WHEN FIND_IN_SET(fk_customer_address_region, @kab_bandung_barat) AND formula_weight >= 2 THEN 'Equal or higher than threshold'
				WHEN FIND_IN_SET(fk_customer_address_region, @kota_bandung) AND formula_weight >= 2 THEN 'Equal or higher than threshold'
				WHEN FIND_IN_SET(fk_customer_address_region, @kab_semarang) AND formula_weight >= 2 THEN 'Equal or higher than threshold'
				WHEN FIND_IN_SET(fk_customer_address_region, @kota_semarang) AND formula_weight >= 2 THEN 'Equal or higher than threshold'
				WHEN FIND_IN_SET(fk_customer_address_region, @kota_surakarta_solo) AND formula_weight >= 2 THEN 'Equal or higher than threshold'
				WHEN FIND_IN_SET(fk_customer_address_region, @kab_bangkalan) AND formula_weight >= 1 THEN 'Equal or higher than threshold'
				WHEN FIND_IN_SET(fk_customer_address_region, @kab_bojonegoro) AND formula_weight >= 1 THEN 'Equal or higher than threshold'
				WHEN FIND_IN_SET(fk_customer_address_region, @kab_gresik) AND formula_weight >= 1 THEN 'Equal or higher than threshold'
				WHEN FIND_IN_SET(fk_customer_address_region, @kab_jombang) AND formula_weight >= 1 THEN 'Equal or higher than threshold'
				WHEN FIND_IN_SET(fk_customer_address_region, @kab_kediri) AND formula_weight >= 1 THEN 'Equal or higher than threshold'
				WHEN FIND_IN_SET(fk_customer_address_region, @kab_lamongan) AND formula_weight >= 1 THEN 'Equal or higher than threshold'
				WHEN FIND_IN_SET(fk_customer_address_region, @kab_lumajang) AND formula_weight >= 1 THEN 'Equal or higher than threshold'
				WHEN FIND_IN_SET(fk_customer_address_region, @kab_madiun) AND formula_weight >= 1 THEN 'Equal or higher than threshold'
				WHEN FIND_IN_SET(fk_customer_address_region, @kab_nganjuk) AND formula_weight >= 1 THEN 'Equal or higher than threshold'
				WHEN FIND_IN_SET(fk_customer_address_region, @kab_pamekasan) AND formula_weight >= 2 THEN 'Equal or higher than threshold'
				WHEN FIND_IN_SET(fk_customer_address_region, @kab_pasuruan) AND formula_weight >= 2 THEN 'Equal or higher than threshold'
				WHEN FIND_IN_SET(fk_customer_address_region, @kab_probolinggo) AND formula_weight >= 1 THEN 'Equal or higher than threshold'
				WHEN FIND_IN_SET(fk_customer_address_region, @kab_sampang) AND formula_weight >= 1 THEN 'Equal or higher than threshold'
				WHEN FIND_IN_SET(fk_customer_address_region, @kab_sidoarjo) AND formula_weight >= 1 THEN 'Equal or higher than threshold'
				WHEN FIND_IN_SET(fk_customer_address_region, @kab_trenggalek) AND formula_weight >= 1 THEN 'Equal or higher than threshold'
				WHEN FIND_IN_SET(fk_customer_address_region, @kab_tuban) AND formula_weight >= 1 THEN 'Equal or higher than threshold'
				WHEN FIND_IN_SET(fk_customer_address_region, @kab_tulungagung) AND formula_weight >= 1 THEN 'Equal or higher than threshold'
				WHEN FIND_IN_SET(fk_customer_address_region, @kota_malang) AND formula_weight >= 2 THEN 'Equal or higher than threshold'
				WHEN FIND_IN_SET(fk_customer_address_region, @kota_surabaya) AND formula_weight >= 2 THEN 'Equal or higher than threshold'
				WHEN FIND_IN_SET(fk_customer_address_region, @kota_balikpapan) AND formula_weight >= 1 THEN 'Equal or higher than threshold'
				WHEN FIND_IN_SET(fk_customer_address_region, @kota_samarinda) AND formula_weight >= 1 THEN 'Equal or higher than threshold'
				WHEN FIND_IN_SET(fk_customer_address_region, @kota_bandar_lampung) AND formula_weight >= 1 THEN 'Equal or higher than threshold'
				WHEN FIND_IN_SET(fk_customer_address_region, @kab_tapanuli_tengah) AND formula_weight >= 1 THEN 'Equal or higher than threshold'
				WHEN FIND_IN_SET(fk_customer_address_region, @kota_medan) AND formula_weight >= 1 THEN 'Equal or higher than threshold'
    ...
    HAVING pass = 1
		AND formula_weight <=7) result) result
