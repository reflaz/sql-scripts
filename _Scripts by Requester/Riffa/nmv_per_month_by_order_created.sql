...
FROM
    (SELECT 
        sup.id_supplier,
            sup.name,
            sup.type,
            IFNULL(sel.tax_class, '') 'tax_class',
            CASE
            ...
            SUM(IF(soish.created_at >= '2016-01-01'
                AND soish.created_at < '2016-02-01', soi.paid_price, 0)) 'nmv_january',
            SUM(IF(soish.created_at >= '2016-02-01'
                AND soish.created_at < '2016-03-01', soi.paid_price, 0)) 'nmv_february',
            SUM(IF(soish.created_at >= '2016-03-01'
                AND soish.created_at < '2016-04-01', soi.paid_price, 0)) 'nmv_march',
            SUM(IF(soish.created_at >= '2016-04-01'
                AND soish.created_at < '2016-05-01', soi.paid_price, 0)) 'nmv_april',
            SUM(IF(soish.created_at >= '2016-05-01'
                AND soish.created_at < '2016-06-01', soi.paid_price, 0)) 'nmv_may',
            SUM(IF(soish.created_at >= '2016-06-01'
                AND soish.created_at < '2016-07-01', soi.paid_price, 0)) 'nmv_june',
            SUM(IF(soish.created_at >= '2016-07-01'
                AND soish.created_at < '2016-08-01', soi.paid_price, 0)) 'nmv_july',
            SUM(IF(soish.created_at >= '2016-08-01'
                AND soish.created_at < '2016-09-01', soi.paid_price, 0)) 'nmv_august',
            SUM(IF(soish.created_at >= '2016-09-01'
                AND soish.created_at < '2016-10-01', soi.paid_price, 0)) 'nmv_september',
            SUM(IF(so.created_at >= '2016-01-01'
                AND so.created_at < '2016-02-01', 1, 0)) 'soi_january',
            SUM(IF(so.created_at >= '2016-02-01'
                AND so.created_at < '2016-03-01', 1, 0)) 'soi_february',
            SUM(IF(so.created_at >= '2016-03-01'
                AND so.created_at < '2016-04-01', 1, 0)) 'soi_march',
            SUM(IF(so.created_at >= '2016-04-01'
                AND so.created_at < '2016-05-01', 1, 0)) 'soi_april',
            SUM(IF(so.created_at >= '2016-05-01'
                AND so.created_at < '2016-06-01', 1, 0)) 'soi_may',
            SUM(IF(so.created_at >= '2016-06-01'
                AND so.created_at < '2016-07-01', 1, 0)) 'soi_june',
            SUM(IF(so.created_at >= '2016-07-01'
                AND so.created_at < '2016-08-01', 1, 0)) 'soi_july',
            SUM(IF(so.created_at >= '2016-08-01'
                AND so.created_at < '2016-09-01', 1, 0)) 'soi_august',
            SUM(IF(so.created_at >= '2016-09-01'
                AND so.created_at < '2016-10-01', 1, 0)) 'soi_september'
    FROM
