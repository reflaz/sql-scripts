...
FROM
    oms_live.ims_sales_order oms_so
        LEFT JOIN
    oms_live.ims_sales_order_item oms_soi ON oms_soi.fk_sales_order = oms_so.id_sales_order
        LEFT JOIN
    oms_live.oms_package_item oms_pi ON oms_pi.fk_sales_order_item = oms_soi.id_sales_order_item
        LEFT JOIN
    oms_live.oms_package oms_pck ON oms_pck.id_package = oms_pi.fk_package
        LEFT JOIN
    oms_live.oms_package_dispatching oms_pd ON oms_pd.fk_package = oms_pck.id_package
        LEFT JOIN
...
WHERE
    oms_so.order_nr IN ('')
GROUP BY oms_pd.tracking_number , oms_so.id_sales_order;