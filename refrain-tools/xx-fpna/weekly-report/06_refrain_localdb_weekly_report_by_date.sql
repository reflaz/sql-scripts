...
###########################################################################################################################################################################
    ...
    FROM
        ...
        bu,
            ...
            SUM(IFNULL(commission_temp, 0)) 'commission',
            SUM(IFNULL(payment_fee_temp, 0)) 'payment_fee',
            SUM(IFNULL(payment_flat_cost, 0) + IFNULL(payment_mdr_cost, 0) + IFNULL(payment_ipp_cost, 0)) 'payment_cost',
            CASE
                WHEN bu IN ('CB' , 'TBC') THEN @cb_delivery_cost * COUNT(bob_id_sales_order_item)
                WHEN bu = 'MA' THEN SUM(IFNULL(delivery_cost, 0) + IFNULL(delivery_cost_discount, 0) + IFNULL(delivery_cost_vat, 0) + IFNULL(insurance_3pl, 0) + IFNULL(insurance_vat_3pl, 0))
                ELSE SUM(IFNULL(total_delivery_cost, 0))
            END 'delivery_cost',
            CASE
                WHEN bu IN ('CB' , 'TBC') THEN @cb_delivery_cost * COUNT(bob_id_sales_order_item)
            ...
            SUM(IFNULL(delivery_cost, 0) + IFNULL(delivery_cost_discount, 0) + IFNULL(delivery_cost_vat, 0)) 'shipment_cost',
            CASE
                ...
                ELSE SUM(IFNULL(delivery_cost, 0) + IFNULL(delivery_cost_discount, 0) + IFNULL(delivery_cost_vat, 0))
            ...
            END 'return_cost',
            CASE
                WHEN bu = 'MA' THEN SUM(IFNULL(pickup_cost, 0) + IFNULL(pickup_cost_discount, 0) + IFNULL(pickup_cost_vat, 0))
                ELSE 0
            ...
            CASE
                ...
                ELSE 0
            END 'cb_charges',
            SUM(IFNULL(seller_charge, 0) + IFNULL(insurance_seller, 0) + IFNULL(insurance_vat_seller, 0)) 'seller_charges',
    ...
    FROM
        ...
        fsoi.*,
            ...
            CASE
                WHEN
                    failed_delivery_date >= @extractstart
                        AND failed_delivery_date < @extractend
                THEN
                    1
                ELSE 0
            END 'failed_flag',
