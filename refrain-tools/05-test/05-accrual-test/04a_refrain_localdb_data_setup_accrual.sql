...
WHERE
    is_actual = 0
		AND status NOT IN ('ACTIVE', 'DELETED');
...
WHERE
    adma.is_actual = 0
		AND adma.status IN ('TEMPORARY' , 'API_TYPE_CONFLICT');
...
WHERE
    addb1.is_actual = 0
		AND addb1.status IN ('TEMPORARY' , 'DUPLICATE');
...
WHERE
    adma1.is_actual = 0
		AND adma1.status IN ('TEMPORARY' , 'DUPLICATE');
...
WHERE
    addb1.is_actual = 0
		AND addb1.status IN ('TEMPORARY' , 'INCOMPLETE')
...
WHERE
    adma1.is_actual = 0
		AND adma1.status IN ('TEMPORARY' , 'INCOMPLETE')
...
WHERE
    addb1.is_actual = 0
		AND addb1.status IN ('TEMPORARY' , 'NO_REFERENCE')
...
WHERE
    adma1.is_actual = 0
		AND adma1.status IN ('TEMPORARY' , 'NO_REFERENCE')
...
WHERE
    is_actual = 0
		AND addb.status IN ('TEMPORARY' , 'NA',
        'INCOMPLETE',
        'COMPLETE',
        'ACTIVE',
        'PACKAGE_NOT_FOUND',
...
WHERE
    is_actual = 0
		AND adma.status IN ('TEMPORARY' , 'NA',
        'INCOMPLETE',
        'COMPLETE',
        'ACTIVE',
        'PACKAGE_NOT_FOUND',
