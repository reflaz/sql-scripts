...
Instructions	: - This script is used to generate EWI report for a specific time frame and seller, because there are several
					sellers that have different account statements time frame (e.g. monthly, rather than weekly)
				  - Copy paste Find id_seller Script below in another tab, then Run it to get specific id_seller
				  - Change @extractstart, @extractend, and @idseller for a specific weekly/monthly time frame 
					and seller before generating the report

-- Run the script below to get a specific seller id based from seller name
-- Find id_seller Script
SELECT
	id_seller
FROM
	screport.seller
WHERE
	name = ''; -- insert the seller name in the quote accordingly to get its seller id 
...
SET @extractstart = '2015-10-01';
SET @extractend = '2015-11-01'; -- this must be the day after
SET @idseller = '951'; -- for specific seller id
-- 16435 Global Deals
-- 951 Planet Sports
...
SELECT 
	...
	(SELECT cc.name_en FROM bob_live.catalog_category cc WHERE cc.lft <= bob_ccat.lft
		AND cc.rgt >= bob_ccat.rgt AND cc.id_catalog_category <> 1 LIMIT 1 , 1) 'category_lv_2'
FROM
    (SELECT 
    ...
    WHERE
		fk_seller = @idseller
			AND start_date >= STR_TO_DATE(@extractstart, '%Y-%m-%d')
    ...
    WHERE
		fk_seller = @idseller
			AND created_at >= STR_TO_DATE(@extractstart, '%Y-%m-%d')
    ...
    oms_live.ims_sales_order_item_status_history oms_ret ON oms_soi.id_sales_order_item = oms_ret.fk_sales_order_item
		...
		LEFT JOIN
	bob_live.sales_order_item bob_soi ON oms_soi.bob_id_sales_order_item = bob_soi.id_sales_order_item
		LEFT JOIN
	...
	bob_live.catalog_category bob_lv2 ON bob_ccat.lft >= bob_lv2.lft
		...
		AND bob_lv2.id_catalog_category = (SELECT cc.id_catalog_category
			FROM bob_live.catalog_category cc
            WHERE cc.lft <= bob_ccat.lft
				AND cc.rgt >= bob_ccat.rgt
                AND cc.id_catalog_category <> 1 LIMIT 1 , 1)
WHERE
