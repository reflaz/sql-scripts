...
SET @extractend = '2016-05-01'; -- this must be the day after the expected end date/h+1
SET @seller = '2372,46669,47453,49534,53448';
...
FROM
    ...
    WHERE
        tr.created_at >= STR_TO_DATE(@extractstart, '%Y-%m-%d')
            ...
            AND FIND_IN_SET(tr.fk_seller, @seller)
...
UNION ALL SELECT 
    ...
    MIN(IF(soish.fk_sales_order_item_status = 27,
        soish.created_at,
        NULL)) 'delivered_date_input',
    MIN(IF(soish.fk_sales_order_item_status = 68,
        soish.created_at,
        NULL)) 'returned_date',
    MIN(IF(soish.fk_sales_order_item_status = 78,
        soish.created_at,
        NULL)) 'replaced_date',
    MIN(IF(soish.fk_sales_order_item_status = 56,
        soish.created_at,
        NULL)) 'refunded_date',
    soi.unit_price,
    soi.paid_price,
...
FROM
    ...
    screport.seller sel ON tr.fk_seller = sel.id_seller
        LEFT JOIN
    screport.sales_order_item scsoi ON tr.ref = scsoi.id_sales_order_item
        LEFT JOIN
    oms_live.ims_sales_order_item soi ON scsoi.src_id = soi.id_sales_order_item
        LEFT JOIN
...
WHERE
    tr.created_at >= STR_TO_DATE(@extractstart, '%Y-%m-%d')
        AND tr.created_at < STR_TO_DATE(@extractend, '%Y-%m-%d')
        AND FIND_IN_SET(tr.fk_seller, @seller)
        AND tr.fk_transaction_type IN (7 , 19, 20, 36, 37)
        AND tr.value IS NOT NULL
GROUP BY scsoi.id_sales_order_item;