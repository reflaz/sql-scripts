...
-- Dates are automatically set
SET @extractend = IF(DAY(NOW()) > 15, DATE_FORMAT(NOW(), '%Y-%m-16'), DATE_FORMAT(NOW(), '%Y-%m-01')); -- this must be the day after
SET @extractstart = IF(DAY(@extractend) > 15, DATE_FORMAT(@extractend, '%Y-%m-01'), DATE_FORMAT(DATE_SUB(@extractend, INTERVAL 1 MONTH), '%Y-%m-16'));
