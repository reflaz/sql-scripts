...
FROM
    (SELECT 
        order_nr 'Order Number',
            ...
            (rounded_chargeable_weight * shipment_cost_rate) 'Total 3PL Charge',
            CASE
                WHEN
                ...
                THEN
                    CASE
                        WHEN (rounded_weight * shipment_cost_rate) - total_shipment_fee_mp_seller - shipping_fee_to_customer <= 0 THEN 0
                        ELSE (rounded_weight * shipment_cost_rate) - total_shipment_fee_mp_seller - shipping_fee_to_customer
                    END
                WHEN (rounded_chargeable_weight * shipment_cost_rate) - total_shipment_fee_mp_seller - shipping_fee_to_customer <= 0 THEN 0
                ELSE (rounded_chargeable_weight * shipment_cost_rate) - total_shipment_fee_mp_seller - shipping_fee_to_customer
            END - shipping_amount 'Shipping Reimbursement/Charge',
            sc_fee_3 'Alraedy Reimbursed/Charged',
            sc_fee_1 'Master Account Charge',
            CASE
                WHEN
                    first_shipped_date >= '2017-01-01'
                        OR order_date >= '2017-01-01'
                ...
                WHEN (rounded_chargeable_weight * shipment_cost_rate) - total_shipment_fee_mp_seller - shipping_fee_to_customer <= 0 THEN 0
                ELSE (rounded_chargeable_weight * shipment_cost_rate) - total_shipment_fee_mp_seller - shipping_fee_to_customer
            END - shipping_amount - sc_fee_3 - sc_fee_1'Offset'
    FROM
