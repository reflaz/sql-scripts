...
SET @extractstart = '2016-12-26';
SET @extractend = '2016-12-27';-- This MUST be D + 1
...
FROM
    (SELECT 
        soi.bob_id_sales_order_item,
            IFNULL(ascsoi.id_sales_order_item, ascsoia.id_sales_order_item) 'sc_sales_order_item',
            so.order_nr,
            so.payment_method,
            soi.sku,
            cc.primary_category,
            ...
            CASE
                ...
                ELSE IFNULL((SELECT 
                    ...
                    LEFT JOIN asc_live.transaction_archive ta ON tr.ref = ta.ref
                        AND tr.number = ta.number
                    WHERE
                        tr.ref = IFNULL(ascsoi.id_sales_order_item, ascsoia.id_sales_order_item)
                            AND tr.fk_transaction_type = 7
                            AND ta.number IS NULL), 0) + IFNULL((SELECT 
                        SUM(IFNULL(ta.value, 0))
                    FROM
                        asc_live.transaction_archive ta
                    WHERE
                        ta.ref = IFNULL(ascsoi.id_sales_order_item, ascsoia.id_sales_order_item)
                            AND ta.fk_transaction_type = 7), 0)
            END 'shipping_fee',
            CASE
                WHEN soi.is_marketplace = 0 THEN NULL
                ELSE IFNULL((SELECT 
                    ...
                    WHERE
                        ta.ref = IFNULL(ascsoi.id_sales_order_item, ascsoia.id_sales_order_item)
                            AND ta.fk_transaction_type = 8), 0)
            ...
            CASE
                ...
                ELSE IFNULL((SELECT 
                    ...
                    WHERE
                        tr.ref = IFNULL(ascsoi.id_sales_order_item, ascsoia.id_sales_order_item)
                            AND tr.fk_transaction_type IN (36 , 37)
                            AND (tr.description LIKE '%reimburse%'
                    ...
                    WHERE
                        ta.ref = IFNULL(ascsoi.id_sales_order_item, ascsoia.id_sales_order_item)
                            AND ta.fk_transaction_type IN (36 , 37)
                            AND (ta.description LIKE '%reimburse%'
                            OR ta.description LIKE '%flat fee%')), 0)
    ...
    HAVING first_shipment_provider IN ('Acommerce')
        AND last_shipment_provider IN ('Acommerce')) result
    ...
    LEFT JOIN asc_live.sales_order_item_archive ascsoia ON soi.id_sales_order_item = ascsoia.src_id
        AND IFNULL(ascsoi.id_sales_order_item, 0) <> ascsoia.id_sales_order_item
