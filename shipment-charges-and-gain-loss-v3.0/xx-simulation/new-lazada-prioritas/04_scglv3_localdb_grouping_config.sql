/*-----------------------------------------------------------------------------------------------------------------------------------
-------------------------------------------------------------------------------------------------------------------------------------
Shipping Charges and Gain/Loss Grouping Using Config Weight
...
FROM
    ...
    FROM
        ...
        ae.bob_id_sales_order_item,
            ...
            IFNULL(CASE
                ...
                WHEN ae.id_campaign IS NOT NULL THEN ae.cam_shipment_fee_mp_seller_flat_rate
            ...
            IFNULL(CASE
                ...
                WHEN ae.id_campaign IS NOT NULL THEN ae.cam_shipment_fee_mp_seller_rate
    ...
    FROM
        ...
        ae.*,
            ...
            cs.rounding_seller,
            cs.rate_card_scheme,
            cs.shipment_fee_mp_seller_threshold,
            ...
            cam.id_campaign,
            cam.campaign,
            cam.shipment_fee_mp_seller_threshold 'cam_shipment_fee_mp_seller_threshold',
            cam.shipment_fee_mp_seller_flat_rate 'cam_shipment_fee_mp_seller_flat_rate',
            cam.shipment_fee_mp_seller_rate 'cam_shipment_fee_mp_seller_rate',
            cam.insurance_rate 'cam_insurance_rate',
            CASE
                WHEN
    ...
    LEFT JOIN zone_mapping zm ON ae.id_district = zm.id_district
        ...
        AND GREATEST(ae.order_date, IFNULL(ae.first_shipped_date, 1)) <= zm.end_date
    LEFT JOIN campaign cam ON ae.fk_campaign = cam.id_campaign
        ...
        AND GREATEST(ae.order_date, IFNULL(ae.first_shipped_date, 1)) <= cam.end_date) ae
