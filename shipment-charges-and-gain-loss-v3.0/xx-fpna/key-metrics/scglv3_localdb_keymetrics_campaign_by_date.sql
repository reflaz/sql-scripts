...
FROM
    (SELECT 
        period,
            ...
            SUM(IF(campaign = 'NON VIP'AND created_at = period, nmv, 0)) 'NMV Non VIP Created',
            
            SUM(IF(campaign = 'VIP' AND shipped_at = period, nmv, 0)) 'NMV VIP Shipped',
            SUM(IF(campaign = 'NON VIP'AND shipped_at = period, nmv, 0)) 'NMV Non VIP Shipped',
            
            SUM(IF(campaign = 'VIP' AND delivered_at = period, nmv, 0)) 'NMV VIP Delivered',
            SUM(IF(campaign = 'NON VIP' AND delivered_at = period, nmv, 0)) 'NMV Non VIP Delivered',
            
            SUM(IF(campaign = 'VIP' AND failed_at = period, nmv, 0)) 'NMV VIP Failed',
            SUM(IF(campaign = 'NON VIP' AND failed_at = period, nmv, 0)) 'NMV Non VIP Failed',
            
            SUM(IF(campaign = 'VIP' AND created_at = period, orders, 0)) 'Orders VIP Created',
            SUM(IF(campaign = 'NON VIP' AND created_at = period, orders, 0)) 'Orders Non VIP Created',
            
            SUM(IF(campaign = 'VIP' AND shipped_at = period, orders, 0)) 'Orders VIP Shipped',
            SUM(IF(campaign = 'NON VIP' AND shipped_at = period, orders, 0)) 'Orders Non VIP Shipped',
            
            SUM(IF(campaign = 'VIP' AND delivered_at = period, orders, 0)) 'Orders VIP Delivered',
            SUM(IF(campaign = 'NON VIP' AND delivered_at = period, orders, 0)) 'Orders Non VIP Delivered',
            
            SUM(IF(campaign = 'VIP' AND failed_at = period, orders, 0)) 'Orders VIP Failed',
            SUM(IF(campaign = 'NON VIP' AND failed_at = period, orders, 0)) 'Orders Non VIP Failed',
            
            SUM(IF(campaign = 'VIP' AND created_at = period, item, 0)) 'Items VIP Created',
            SUM(IF(campaign = 'NON VIP' AND created_at = period, item, 0)) 'Items Non VIP Created',
            
            SUM(IF(campaign = 'VIP' AND shipped_at = period, item, 0)) 'Items VIP Shipped',
            SUM(IF(campaign = 'NON VIP' AND shipped_at = period, item, 0)) 'Items Non VIP Shipped',
            
            SUM(IF(campaign = 'VIP' AND delivered_at = period, item, 0)) 'Items VIP Delivered',
            SUM(IF(campaign = 'NON VIP' AND delivered_at = period, item, 0)) 'Items Non VIP Delivered',
            
            SUM(IF(campaign = 'VIP' AND failed_at = period, item, 0)) 'Items VIP Failed',
            SUM(IF(campaign = 'NON VIP' AND failed_at = period, item, 0)) 'Items Non VIP Failed',
            
            SUM(IF(campaign = 'VIP' AND created_at = period, parcels, 0)) 'Parcels VIP Created',
            SUM(IF(campaign = 'NON VIP' AND created_at = period, parcels, 0)) 'Parcels Non VIP Created',
            
            SUM(IF(campaign = 'VIP' AND shipped_at = period, parcels, 0)) 'Parcels VIP Shipped',
            SUM(IF(campaign = 'NON VIP' AND shipped_at = period, parcels, 0)) 'Parcels Non VIP Shipped',
            
            SUM(IF(campaign = 'VIP' AND delivered_at = period, parcels, 0)) 'Parcels VIP Delivered',
            SUM(IF(campaign = 'NON VIP' AND delivered_at = period, parcels, 0)) 'Parcels Non VIP Delivered',
            
            SUM(IF(campaign = 'VIP' AND failed_at = period, parcels, 0)) 'Parcels VIP Failed',
            SUM(IF(campaign = 'NON VIP' AND failed_at = period, parcels, 0)) 'Parcels Non VIP Failed',
            
            SUM(IF(campaign = 'VIP' AND created_at = period, weight, 0)) 'Weight VIP Created',
            SUM(IF(campaign = 'NON VIP' AND created_at = period, weight, 0)) 'Weight Non VIP Created',
            
            SUM(IF(campaign = 'VIP' AND shipped_at = period, weight, 0)) 'Weight VIP Shipped',
            SUM(IF(campaign = 'NON VIP' AND shipped_at = period, weight, 0)) 'Weight Non VIP Shipped',
            
            SUM(IF(campaign = 'VIP' AND delivered_at = period, weight, 0)) 'Weight VIP Delivered',
            SUM(IF(campaign = 'NON VIP' AND delivered_at = period, weight, 0)) 'Weight Non VIP Delivered',
            
            SUM(IF(campaign = 'VIP' AND failed_at = period, weight, 0)) 'Weight VIP Failed',
            SUM(IF(campaign = 'NON VIP' AND failed_at = period, weight, 0)) 'Weight Non VIP Failed',
            
            SUM(IF(campaign = 'VIP' AND created_at = period, chargable_weight, 0)) 'Chargable Weight VIP Created',
            SUM(IF(campaign = 'NON VIP' AND created_at = period, chargable_weight, 0)) 'Chargable Weight Non VIP Created',
            
            SUM(IF(campaign = 'VIP' AND shipped_at = period, chargable_weight, 0)) 'Chargable Weight VIP Shipped',
            SUM(IF(campaign = 'NON VIP' AND shipped_at = period, chargable_weight, 0)) 'Chargable Weight Non VIP Shipped',
            
            SUM(IF(campaign = 'VIP' AND delivered_at = period, chargable_weight, 0)) 'Chargable Weight VIP Delivered',
            SUM(IF(campaign = 'NON VIP' AND delivered_at = period, chargable_weight, 0)) 'Chargable Weight Non VIP Delivered',
            
            SUM(IF(campaign = 'VIP' AND failed_at = period, chargable_weight, 0)) 'Chargable Weight VIP Failed',
            SUM(IF(campaign = 'NON VIP' AND failed_at = period, chargable_weight, 0)) 'Chargable Weight Non VIP Failed',
            
            SUM(IF(campaign = 'VIP' AND created_at = period, seller_charges, 0)) 'Seller Charges VIP Created',
            SUM(IF(campaign = 'NON VIP' AND created_at = period, seller_charges, 0)) 'Seller Charges Non VIP Created',
            
            SUM(IF(campaign = 'VIP' AND shipped_at = period, seller_charges, 0)) 'Seller Charges VIP Shipped',
            SUM(IF(campaign = 'NON VIP' AND shipped_at = period, seller_charges, 0)) 'Seller Charges Non VIP Shipped',
            
            SUM(IF(campaign = 'VIP' AND delivered_at = period, seller_charges, 0)) 'Seller Charges VIP Delivered',
            SUM(IF(campaign = 'NON VIP' AND delivered_at = period, seller_charges, 0)) 'Seller Charges Non VIP Delivered',
            
            SUM(IF(campaign = 'VIP' AND failed_at = period, seller_charges, 0)) 'Seller Charges VIP Failed',
            SUM(IF(campaign = 'NON VIP' AND failed_at = period, seller_charges, 0)) 'Seller Charges Non VIP Failed',
            
            SUM(IF(campaign = 'VIP' AND created_at = period, customer_charges, 0)) 'Customer Charges VIP Created',
            SUM(IF(campaign = 'NON VIP' AND created_at = period, customer_charges, 0)) 'Customer Charges Non VIP Created',
            
            SUM(IF(campaign = 'VIP' AND shipped_at = period, customer_charges, 0)) 'Customer Charges VIP Shipped',
            SUM(IF(campaign = 'NON VIP' AND shipped_at = period, customer_charges, 0)) 'Customer Charges Non VIP Shipped',
            
            SUM(IF(campaign = 'VIP' AND delivered_at = period, customer_charges, 0)) 'Customer Charges VIP Delivered',
            SUM(IF(campaign = 'NON VIP' AND delivered_at = period, customer_charges, 0)) 'Customer Charges Non VIP Delivered',
            
            SUM(IF(campaign = 'VIP' AND failed_at = period, customer_charges, 0)) 'Customer Charges VIP Failed',
            SUM(IF(campaign = 'NON VIP' AND failed_at = period, customer_charges, 0)) 'Customer Charges Non VIP Failed',
            
            SUM(IF(campaign = 'VIP' AND created_at = period, delivery_cost, 0)) 'Delivery Cost VIP Created',
            SUM(IF(campaign = 'NON VIP' AND created_at = period, delivery_cost, 0)) 'Delivery Cost Non VIP Created',
            
            SUM(IF(campaign = 'VIP' AND shipped_at = period, delivery_cost, 0)) 'Delivery Cost VIP Shipped',
            SUM(IF(campaign = 'NON VIP' AND shipped_at = period, delivery_cost, 0)) 'Delivery Cost Non VIP Shipped',
            
            SUM(IF(campaign = 'VIP' AND delivered_at = period, delivery_cost, 0)) 'Delivery Cost VIP Delivered',
            SUM(IF(campaign = 'NON VIP' AND delivered_at = period, delivery_cost, 0)) 'Delivery Cost Non VIP Delivered',
            
            SUM(IF(campaign = 'VIP' AND failed_at = period, delivery_cost, 0)) 'Delivery Cost VIP Failed',
            SUM(IF(campaign = 'NON VIP' AND failed_at = period, delivery_cost, 0)) 'Delivery Cost Non VIP Failed',
            
            SUM(IF(campaign = 'VIP' AND created_at = period, pickup_cost, 0)) 'Pickup Cost VIP Created',
            SUM(IF(campaign = 'NON VIP' AND created_at = period, pickup_cost, 0)) 'Pickup Cost Non VIP Created',
            
            SUM(IF(campaign = 'VIP' AND shipped_at = period, pickup_cost, 0)) 'Pickup Cost VIP Shipped',
            SUM(IF(campaign = 'NON VIP' AND shipped_at = period, pickup_cost, 0)) 'Pickup Cost Non VIP Shipped',
            
            SUM(IF(campaign = 'VIP' AND delivered_at = period, pickup_cost, 0)) 'Pickup Cost VIP Delivered',
            SUM(IF(campaign = 'NON VIP' AND delivered_at = period, pickup_cost, 0)) 'Pickup Cost Non VIP Delivered',
            
            SUM(IF(campaign = 'VIP' AND failed_at = period, pickup_cost, 0)) 'Pickup Cost VIP Failed',
            SUM(IF(campaign = 'NON VIP' AND failed_at = period, pickup_cost, 0)) 'Pickup Cost Non VIP Failed'
    FROM
        (SELECT 
        bu,
