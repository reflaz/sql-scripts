...
SELECT 
    ...
    seller_name,
    tax_class,
    SUM(commission_should_be) 'total_commission_should_be'
FROM
    (SELECT 
        oms.order_nr,
            ...
            oms.commission,
            IFNULL(oms.commission, IFNULL(gc.general_commission, rc.general_commission) * unit_price / - 100) 'commission_should_be',
            oms.payment_fee,
    ...
    FROM
        oms
    LEFT JOIN commission_tree_mapping gc ON oms.primary_category = gc.lookup_cat_id
        AND oms.tax_class = gc.tax_class
    LEFT JOIN commission_tree_mapping rc ON oms.tax_class = rc.tax_class
