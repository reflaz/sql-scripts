...
FROM
    (SELECT 
        tr.number 'transaction_id',
            ...
            oms_soi.coupon_money_value,
            oms_soi.cart_rule_discount,
            ...
            SUBSTRING_INDEX(SUBSTRING_INDEX(sso.address_shipping, 'first_name":"', - 1), '","', 1) 'first_name',
            SUBSTRING_INDEX(SUBSTRING_INDEX(sso.address_shipping, 'address1":"', - 1), '","', 1) 'address1',
            SUBSTRING_INDEX(SUBSTRING_INDEX(sso.address_shipping, 'city":"', - 1), '","', 1) 'city'
    FROM
        asc_live.transaction tr
    LEFT JOIN asc_live.sales_order_item ssoi ON tr.ref = ssoi.id_sales_order_item
    LEFT JOIN asc_live.sales_order sso ON ssoi.fk_sales_order = sso.id_sales_order
