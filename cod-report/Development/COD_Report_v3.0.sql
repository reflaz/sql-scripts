...
Instructions	: - Change @extractstart and @extractend for a specific weekly/monthly time frame before generating the report
				  - Change @sellertype accordingly (supplier/merchant)
-------------------------------------------------------------------------------------------------------------------------------------
-----------------------------------------------------------------------------------------------------------------------------------*/
...
SET @extractend = '2016-04-01'; -- this must be the day after the expected end date/h+1
SET @sellertype = 'merchant'; -- supplier/merchant
...
SELECT 
    ...
    oms_so.supplier_name 'Supplier Name',
    oms_so.supplier_type 'Supplier Type'
FROM
    (SELECT 
        so.order_nr,
            so.created_at,
            ...
            (LENGTH(GROUP_CONCAT(pckshp.name)) - LENGTH(REPLACE(GROUP_CONCAT(pckshp.name), 'shipped', ''))) / LENGTH('shipped') AS 'times_shipped',
            soi.name 'item_name',
    ...
    oms_live.oms_package oms_pck ON oms_pi.fk_package = oms_pck.id_package
        LEFT JOIN
    oms_live.ims_sales_order_item_status_history oms_ship ON oms_so.id_sales_order_item = oms_ship.fk_sales_order_item
        AND oms_ship.id_sales_order_item_status_history = (SELECT 
            MIN(id_sales_order_item_status_history)
        FROM
            oms_live.ims_sales_order_item_status_history
        WHERE
            fk_sales_order_item = oms_so.id_sales_order_item
                AND fk_sales_order_item_status = 5)
        LEFT JOIN
    oms_live.ims_sales_order_item_status_history oms_delv ON oms_so.id_sales_order_item = oms_delv.fk_sales_order_item
    ...
    oms_live.oms_package_dispatching_history oms_pd1 ON oms_pck.id_package = oms_pd1.fk_package
        ...
        WHERE
            fk_package = oms_pck.id_package
                AND fk_shipment_provider IS NOT NULL
    ...
    oms_live.oms_package_dispatching_history oms_pd3 ON oms_pck.id_package = oms_pd3.fk_package
        ...
        LIMIT 2 , 1)
        LEFT JOIN
    oms_live.ims_user oms_user ON oms_delv.fk_user = oms_user.id_user
        LEFT JOIN
