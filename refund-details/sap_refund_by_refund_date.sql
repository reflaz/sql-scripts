...
FROM
    ims_sales_order so
        JOIN
    ims_sales_order_item soi ON so.id_sales_order = soi.fk_sales_order
        JOIN
    (SELECT 
        fk_sales_order_item, MIN(created_at) AS created_at
    FROM
        ims_sales_order_item_status_history
    WHERE
        fk_sales_order_item_status = 56
    GROUP BY fk_sales_order_item
    HAVING MIN(created_at) >= '2016-02-01'
        AND MIN(created_at) < '2016-03-01') soish ON soi.id_sales_order_item = soish.fk_sales_order_item
        JOIN
