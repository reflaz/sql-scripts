...
SET @bu = 'MA'; #Option MA, DB, Retail, Cross Border
SET @cut_off_date = '2017-03-01';
...
FROM
    (SELECT 
        ...
        @cut_off_date 'Cut Off Date',
            SUM(IF(business_unit = @bu, nmv, 0)) 'NMV',
            SUM(IF(business_unit = @bu, item, 0)) 'Item',
            SUM(IF(business_unit = @bu, sales_order, 0)) 'Order',
            SUM(IF(business_unit = @bu, package, 0)) 'Package',
            SUM(IF(business_unit = @bu, weight, 0)) 'Weight',
            SUM(IF(business_unit = @bu, pickup_weight, 0)) 'Pickup Weight',
            SUM(IF(business_unit = @bu, failed_delivery_item, 0)) 'Failed Item',
            SUM(IF(business_unit = @bu, failed_delivery_item, 0))/SUM(IF(business_unit = @bu, item, 0)) 'FDR',
            SUM(IF(business_unit = @bu, failed_delivery_cost, 0))/SUM(IF(business_unit = @bu, normalised_failed_delivery_item, 0))*SUM(IF(business_unit = @bu, failed_delivery_item, 0)) 'Failed Delivery Cost',
            SUM(IF(business_unit = @bu, failed_delivery_cost, 0))/SUM(IF(business_unit = @bu, normalised_failed_delivery_item, 0)) 'Failed Delivery Cost per Item',            
            SUM(IF(business_unit = @bu, seller_charges, 0)) 'Seller Charges',
            SUM(IF(business_unit = @bu, customer_charges, 0)) 'Customer Charges',
            SUM(IF(business_unit = @bu, normalised_shipping_cost, 0))/SUM(IF(business_unit = @bu, normalised_item, 0))*SUM(IF(business_unit = @bu, item, 0))  'Shipping Cost',
            SUM(IF(business_unit = @bu, shipping_surcharge, 0)) 'Shipping Surcharge',
            SUM(IF(business_unit = @bu, shipping_amount, 0)) 'Shipping Amount',
            SUM(IF(business_unit = @bu, normalised_delivery_cost, 0))/SUM(IF(business_unit = @bu, normalised_item, 0))*SUM(IF(business_unit = @bu, item, 0)) 'Delivery Cost',
            SUM(IF(business_unit = @bu, normalised_pc1, 0))/SUM(IF(business_unit = @bu, normalised_item, 0))*SUM(IF(business_unit = @bu, item, 0)) 'PC1',
            SUM(IF(business_unit = @bu, normalised_pc1, 0))/SUM(IF(business_unit = @bu, normalised_item, 0)) 'PC1 per Item',
            SUM(IF(business_unit = @bu, normalised_pc2, 0))/SUM(IF(business_unit = @bu, normalised_item, 0))*SUM(IF(business_unit = @bu, item, 0)) 'PC2',
            SUM(IF(business_unit = @bu, normalised_pc2, 0))/SUM(IF(business_unit = @bu, normalised_item, 0)) 'PC2 per Item',
            SUM(IF(business_unit = @bu, normalised_pc3, 0))/SUM(IF(business_unit = @bu, normalised_item, 0))*SUM(IF(business_unit = @bu, item, 0)) 'PC3',
            SUM(IF(business_unit = @bu, normalised_pc3, 0))/SUM(IF(business_unit = @bu, normalised_item, 0)) 'PC3 per Item',
            SUM(IF(business_unit = @bu, normalised_gain_loss, 0))/SUM(IF(business_unit = @bu, normalised_item, 0))*SUM(IF(business_unit = @bu, item, 0)) 'Gain/Loss',
            SUM(IF(business_unit = @bu, normalised_gain_loss, 0))/SUM(IF(business_unit = @bu, normalised_item, 0)) 'Gain/Loss per Item'
    ...
    FROM
        ...
        fk_shipment_scheme,
            ...
            COUNT(DISTINCT package_number) 'package',
            SUM(item) 'item',
            SUM(rounded_weight) 'weight',
            SUM(pickup_weight) 'pickup_weight',
            SUM(total_shipment_fee_mp_seller) 'seller_charges',
            SUM(shipping_fee_to_customer) 'customer_charges',
            SUM(shipping_cost) 'shipping_cost',
            ...
            SUM(if(pass = 1, shipping_cost,0)) 'normalised_shipping_cost',
    ...
    FROM
        ...
        *,
            ...
            END 'pass',
            CASE
				WHEN total_pickup_cost <> 0 THEN rounded_weight
			END 'pickup_weight',
            CASE WHEN order_date < @cut_off_date THEN 'Created Before' ELSE 'Created After' END 'period',
            CASE
            ...
            qty 'item',
            total_shipment_cost 'shipping_cost',
            total_insurance_charge 'insurance',
            CASE
    ...
    GROUP BY business_unit , period) ac
