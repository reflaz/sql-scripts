/*-----------------------------------------------------------------------------------------------------------------------------------
-------------------------------------------------------------------------------------------------------------------------------------
SCGL Key Metrics Summarization - Weight vs Local MP Business Unit
...
FROM
    (SELECT 
        weight_bucket,
            SUM(IF(shipment_scheme = 'Direct Billing', nmv, 0)) 'nmv_db',
            SUM(IF(shipment_scheme = 'Direct Billing', nmv, 0)) / MAX(IF(shipment_scheme = 'Direct Billing', total_nmv, NULL)) 'nmv_db_proportion',
            SUM(IF(shipment_scheme = 'Direct Billing', count_soi, 0)) 'count_soi_db',
            SUM(IF(shipment_scheme = 'Direct Billing', count_soi, 0)) / MAX(IF(shipment_scheme = 'Direct Billing', total_count_soi, NULL)) 'count_soi_db_proportion',
            SUM(IF(shipment_scheme = 'Direct Billing', gain_loss, 0)) 'gain_loss_db',
            SUM(IF(shipment_scheme = 'Direct Billing', gain_loss, 0)) / MAX(IF(shipment_scheme = 'Direct Billing', total_gain_loss, NULL)) 'gain_loss_db_proportion',
            SUM(IF(shipment_scheme = 'Direct Billing', gain_loss, 0)) / SUM(IF(shipment_scheme = 'Direct Billing', count_soi, 0)) 'cost_per_item_db',
            SUM(IF(shipment_scheme = 'Master Account', nmv, 0)) 'nmv_ma',
            SUM(IF(shipment_scheme = 'Master Account', nmv, 0)) / MAX(IF(shipment_scheme = 'Master Account', total_nmv, NULL)) 'nmv_ma_proportion',
            SUM(IF(shipment_scheme = 'Master Account', count_soi, 0)) 'count_soi_ma',
            SUM(IF(shipment_scheme = 'Master Account', count_soi, 0)) / MAX(IF(shipment_scheme = 'Master Account', total_count_soi, NULL)) 'count_soi_ma_proportion',
            SUM(IF(shipment_scheme = 'Master Account', gain_loss, 0)) 'gain_loss_ma',
            SUM(IF(shipment_scheme = 'Master Account', gain_loss, 0)) / MAX(IF(shipment_scheme = 'Master Account', total_gain_loss, NULL)) 'gain_loss_ma_proportion',
            SUM(IF(shipment_scheme = 'Master Account', gain_loss, 0)) / SUM(IF(shipment_scheme = 'Master Account', count_soi, 0)) 'cost_per_item_ma',
            SUM(IF(shipment_scheme = 'FBL', nmv, 0)) 'nmv_fbl',
            SUM(IF(shipment_scheme = 'FBL', nmv, 0)) / MAX(IF(shipment_scheme = 'FBL', total_nmv, NULL)) 'nmv_fbl_proportion',
            SUM(IF(shipment_scheme = 'FBL', count_soi, 0)) 'count_soi_fbl',
            SUM(IF(shipment_scheme = 'FBL', count_soi, 0)) / MAX(IF(shipment_scheme = 'FBL', total_count_soi, NULL)) 'count_soi_fbl_proportion',
            SUM(IF(shipment_scheme = 'FBL', gain_loss, 0)) 'gain_loss_fbl',
            SUM(IF(shipment_scheme = 'FBL', gain_loss, 0)) / MAX(IF(shipment_scheme = 'FBL', total_gain_loss, NULL)) 'gain_loss_fbl_proportion',
            SUM(IF(shipment_scheme = 'FBL', gain_loss, 0)) / SUM(IF(shipment_scheme = 'FBL', count_soi, 0)) 'cost_per_item_fbl',
            SUM(nmv) 'nmv',
    ...
    FROM
        ...
        *,
            ...
            IFNULL(total_shipment_fee_mp_seller, 0) + IFNULL(shipping_fee_to_customer, 0) - IFNULL(total_delivery_cost, 0) - IF(fk_shipment_scheme IN (2 , 3, 4)
    ...
    HAVING pass = 1) ac
    GROUP BY weight_bucket , shipment_scheme) ac
    LEFT JOIN (SELECT 
        fk_shipment_scheme,
            shipment_scheme,
            business_unit,
    ...
    GROUP BY shipment_scheme) totals ON ac.shipment_scheme = totals.shipment_scheme UNION ALL SELECT 
        fk_shipment_scheme,
            shipment_scheme,
            business_unit,
    ...
    WHERE
        (not_delivered_date >= @extractstart
            ...
            AND FIND_IN_SET(fk_shipment_scheme, @id_shipment_scheme)
    HAVING pass = 1) ac
    GROUP BY total_weight_bucket , shipment_scheme) result
