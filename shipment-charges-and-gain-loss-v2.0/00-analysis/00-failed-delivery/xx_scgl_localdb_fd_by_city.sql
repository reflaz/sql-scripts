/*-----------------------------------------------------------------------------------------------------------------------------------
-------------------------------------------------------------------------------------------------------------------------------------
SCGL Key Metrics Summarization - Failed Delivery
...
SET @extractend = '2017-02-13'; -- This MUST be D + 1
...
FROM
    (SELECT 
        fz.region,
            fz.city,
            zone_type,
            COUNT(DISTINCT IF(payment_method = 'CashOnDelivery'
                AND failed_delivery = 1, package_number, NULL)) 'failed_delivery_package_cod',
            COUNT(DISTINCT IF(payment_method = 'CashOnDelivery', package_number, NULL)) 'total_package_cod',
            COUNT(DISTINCT IF(payment_method = 'CashOnDelivery'
                AND failed_delivery = 1, package_number, NULL)) / COUNT(DISTINCT IF(payment_method = 'CashOnDelivery', package_number, NULL)) 'failed_delivery_rate_cod',
            SUM(IF(payment_method = 'CashOnDelivery'
                AND failed_delivery = 1, total_shipment_cost, 0)) 'failed_delivery_shipment_cost_cod',
            COUNT(DISTINCT IF(payment_method <> 'CashOnDelivery'
                AND failed_delivery = 1, package_number, NULL)) 'failed_delivery_package_non_cod',
            COUNT(DISTINCT IF(payment_method <> 'CashOnDelivery', package_number, NULL)) 'total_package_non_cod',
            COUNT(DISTINCT IF(payment_method <> 'CashOnDelivery'
                AND failed_delivery = 1, package_number, NULL)) / COUNT(DISTINCT IF(payment_method <> 'CashOnDelivery', package_number, NULL)) 'failed_delivery_rate_non_cod',
            SUM(IF(payment_method <> 'CashOnDelivery'
                AND failed_delivery = 1, total_shipment_cost, 0)) 'failed_delivery_shipment_cost_non_cod'
    FROM
        (SELECT 
        *,
            CASE
            ...
            IF(fk_shipment_scheme IN (2 , 3, 4)
                AND first_shipped_date IS NOT NULL
                AND not_delivered_date IS NOT NULL, 1, 0) 'failed_delivery'
    FROM
    ...
    LEFT JOIN scgl.free_zone fz ON result.id_district = fz.id_district
    GROUP BY fz.id_city) result