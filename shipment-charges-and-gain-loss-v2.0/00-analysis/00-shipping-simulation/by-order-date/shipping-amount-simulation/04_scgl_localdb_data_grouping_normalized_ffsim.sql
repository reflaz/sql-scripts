...
SELECT 
    ...
    CASE
        WHEN is_marketplace = 0 THEN (IFNULL(temp_shipping_amount, 0) / 1.1) + (IFNULL(temp_shipping_surcharge, 0) / 1.1)
        ELSE IFNULL(temp_shipping_amount, 0) + IFNULL(temp_shipping_surcharge, 0)
...
FROM
    (SELECT 
        *,
            ...
            SUM(IFNULL(shipping_amount_new, 0)) 'temp_shipping_amount',
            SUM(IFNULL(shipping_surcharge_old, 0)) 'temp_shipping_surcharge_old',
            SUM(IFNULL(shipping_surcharge_new, 0)) 'temp_shipping_surcharge',
    ...
    FROM
        ...
        ae.bob_id_sales_order_item,
            ...
            -- CASE
                -- WHEN fk_shipment_scheme IN (0 , 5) THEN 0
                -- WHEN delivery_type IN ('express' , 'nextday', 'sameday') THEN shipping_amount
                -- ELSE
					IFNULL((SELECT 
                        CASE
                    ...
                    WHERE
                        dd.order_nr = ae.order_nr
                            -- AND (dd.seller_type = 'supplier'
                            -- OR dd.tax_class <> 'international')
                            -- AND dd.delivery_type <> 'digital'
                            -- AND dd.first_shipment_provider <> 'Digital Delivery'
                            -- AND dd.last_shipment_provider <> 'Digital Delivery'
