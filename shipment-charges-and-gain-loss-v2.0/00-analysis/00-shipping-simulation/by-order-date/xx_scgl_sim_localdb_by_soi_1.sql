...
SET @extractend = '2016-05-01';-- This MUST be D + 1
...
SELECT 
    ...
    IF(bu = 'CB', 'Cross Border', ac.origin) 'origin',
    ac.id_region,
    ac.region,
    ac.id_city,
    ac.city,
    ...
    (SELECT 
            MIN(shipment_cost_rate)
        FROM
            scgl.rate_card_customer rcc
        WHERE
            rcc.id_district = ac.id_district
                AND rcc.origin = ac.origin
                AND rcc.active = ac.active
                AND SUM(ac.customer_weight) < weight_break) 'customer_rate_card',
...
FROM
    (SELECT 
        ac.order_nr,
            ...
            CASE
                ...
                WHEN FIND_IN_SET(bob_id_supplier, @vip) THEN 'MP Local - VIP'
    ...
    WHERE
        ac.order_date >= @extractstart
            ...
            AND ac.fk_shipment_scheme IN (1 , 2, 3, 4, 5, 7)
    HAVING pass = 1) ac
