...
SELECT 
    ...
    region,
    zone_type 'zone_type',
    shipment_cost_rate 'customer_rate_per_kg',
    SUM(paid_price) 'paid_price',
    COUNT(DISTINCT order_nr) 'count_so',
    SUM(qty) 'count_soi',
    SUM(shipping_amount_new) 'total_shipping_amount',
    SUM(shipping_surcharge_new) 'total_shipping_surcharge'
FROM
    (SELECT 
    ...
    WHERE
        ae.fk_shipment_scheme IN (1 , 2, 3, 4, 7)
    HAVING pass = 1) ae
GROUP BY id_city