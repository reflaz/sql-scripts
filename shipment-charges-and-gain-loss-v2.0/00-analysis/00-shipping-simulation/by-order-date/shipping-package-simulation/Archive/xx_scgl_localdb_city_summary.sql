...
SELECT 
    ...
    SUM(qty) 'count_soi',
    SUM(rounded_weight) 'total_weight',
    ...
    SUM(total_fd_cost) 'total_fd_cost',
...
FROM
    (SELECT 
        ac.order_nr,
			paid_price,
            IFNULL(CASE
                WHEN ac.is_marketplace = 0 THEN ac.shipping_amount / 1.1
                ELSE ac.shipping_amount
            END, 0) 'shipping_amount',
            IFNULL(CASE
                WHEN ac.is_marketplace = 0 THEN ac.shipping_surcharge / 1.1
                ELSE ac.shipping_surcharge
            END, 0) 'shipping_surcharge',
            ac.shipping_fee_to_customer,
            ac.total_shipment_fee_mp_seller,
            - ac.total_delivery_cost 'total_delivery_cost',
            CASE
                WHEN ac.not_delivered_date IS NOT NULL THEN - total_shipment_cost
                ELSE 0
            END 'total_fd_cost',
            ac.rounded_weight,
            ...
            CASE
                WHEN ac.fk_shipment_scheme = 5 THEN @cbrate * qty
                WHEN ac.not_delivered_date IS NOT NULL THEN total_shipment_fee_mp_seller + shipping_fee_to_customer - total_delivery_cost - total_shipment_cost
            ...
            fz.district
    FROM
        scgl.anondb_calculate_ffsim ac
