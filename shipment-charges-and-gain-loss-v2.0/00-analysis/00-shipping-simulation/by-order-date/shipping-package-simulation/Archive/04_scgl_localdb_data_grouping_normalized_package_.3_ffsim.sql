...
TRUNCATE TABLE anondb_calculate_ffsim;

INSERT INTO anondb_calculate_ffsim
SELECT 
    ...
    unit_price,
    paid_price,
    shipping_amount_new AS shipping_amount,
    shipping_surcharge_old,
    shipping_surcharge_new AS shipping_surcharge,
    CASE
        WHEN is_marketplace = 0 THEN (IFNULL(shipping_amount_new, 0) / 1.1) + (IFNULL(shipping_surcharge_new, 0) / 1.1)
        ELSE IFNULL(shipping_amount_new, 0) + IFNULL(shipping_surcharge_new, 0)
    ...
    retail_cogs,
    payment_cost_logic,
    sc_fee_1 AS shipping_fee,
    sc_fee_2 AS shipping_fee_credit,
    sc_fee_3 AS seller_cr_db_item,
    qty,
...
FROM
	...
	SELECT 
		...
		IFNULL((SELECT 
						CASE
								WHEN SUM(dd.unit_price) < rcc.flat_rate AND zt.id_zone_type = 4 THEN 5000
					...
					FROM
						scgl.anondb_extract dd
					WHERE
						dd.order_nr = ae.order_nr),
				0) * qty 'shipping_amount_new',
	...
	FROM
        ...
        -- GROUP BY order_nr , bob_id_supplier , id_package_dispatching) ae
			LEFT JOIN
		scgl.rate_card_customer rcc ON ae.id_district = rcc.id_district
			AND ae.origin = rcc.origin
			AND ae.formula_weight <= rcc.weight_break
			AND ae.active = rcc.active
			LEFT JOIN
		scgl.free_zone fz ON rcc.id_district = fz.id_district
			AND rcc.active = fz.active
			LEFT JOIN
		scgl.zone_type zt ON fz.fk_zone_type = zt.id_zone_type) ae;