...
SELECT 
    ...
    ac.gain_loss,
    CEIL(ac.source_formula_weight) 'source_rounded_weight',
    CASE
        WHEN
    ...
    IFNULL(IF(ac.is_source_package_weight,
                source_package_weight,
                source_dim_weight) / ac.source_formula_weight,
            0) 'package_weight_percentage',
    ...
    IFNULL(rcc.shipment_cost_rate, 0) * IFNULL(IF(ac.is_source_package_weight,
                source_package_weight,
                source_dim_weight) / ac.source_formula_weight,
            0) * CASE
        WHEN
    ...
    (ac.shipping_amount / IF(seller_type = 'supplier', 1.1, 1)) + (IFNULL(rcc.shipment_cost_rate, 0) * IFNULL(IF(ac.is_source_package_weight,
                source_package_weight,
                source_dim_weight) / ac.source_formula_weight,
            0) * CASE
        WHEN
