...
SELECT 
    ...
    SUM(qty) 'qty',
    SUM(IF(shipping_surcharge = 0
            AND source_shipping_surcharge > 0,
        1,
        0)) 'qty_become_charged',
    SUM(total_delivery_cost) 'total_delivery_cost',
    SUM(total_failed_delivery) 'total_failed_delivery_cost',
    SUM(total_shipment_fee_mp_seller) 'total_shipment_fee_mp_seller',
    SUM(shipping_fee_to_customer) 'current_shipping_fee_to_customer',
    SUM(gain_loss) 'current_gain_loss',
    SUM(source_shipping_amount + source_shipping_surcharge) 'to_be_shipping_fee_to_customer',
    SUM(gain_loss_source) 'to_be_gain_loss',
    SUM(gain_loss_source - gain_loss) 'gain_loss_difference'
FROM
    (SELECT 
        ac.order_nr,
            ...
            rcc.id_district,
            rcc.district,
            ac.zone_type_temp 'zone_type',
            ac.shipping_amount,
            ac.shipping_surcharge,
            ac.shipping_fee_to_customer,
            ac.rounded_weight,
            ac.rounded_chargeable_weight,
            ac.total_shipment_fee_mp_seller,
            ac.total_pickup_cost,
            ac.total_insurance_charge,
            ac.total_delivery_cost,
            ac.total_failed_delivery,
            ac.gain_loss,
            CEIL(ac.source_formula_weight) 'source_chargeable_weight',
            CASE
                WHEN
                    ac.fk_shipment_scheme IN (1 , 2, 3, 4)
                THEN
                    CASE
                        WHEN
                            ac.first_shipped_date < '2017-01-01'
                                AND ac.order_date < '2017-01-01'
                                AND ac.fk_shipment_scheme = 1
                                AND ac.source_formula_weight > 7.3
                        THEN
                            0
                        WHEN
                        ...
                        THEN
                            CASE
                                WHEN
                                    ac.first_shipped_date >= '2017-01-01'
                                        OR ac.order_date >= '2017-01-01' = 1
                                THEN
                                    CASE
                                        WHEN CEIL(ac.source_formula_weight) <= 2 THEN 0
                                        ELSE CEIL(ac.source_formula_weight) - 2
                                    END
                                ELSE CASE
                                    WHEN CEIL(ac.source_formula_weight) <= 7 THEN 0
                                    ELSE CEIL(ac.source_formula_weight)
                                END
                            END
                        ELSE CEIL(ac.source_formula_weight)
                    END
                ELSE 0
            END 'source_rounded_chargeable_weight',
            ac.source_dim_weight,
            ac.source_package_weight,
            ac.source_formula_weight,
            ac.is_source_package_weight,
            IFNULL(IF(ac.is_source_package_weight, source_package_weight, source_dim_weight) / ac.source_formula_weight, 0) 'package_weight_percentage',
            rcc.shipment_cost_rate 'shipping_charge_customer_rate',
            ac.shipping_amount / IF(seller_type = 'supplier', 1.1, 1) 'source_shipping_amount',
            IFNULL(rcc.shipment_cost_rate, 0) * IFNULL(IF(ac.is_source_package_weight, source_package_weight, source_dim_weight) / ac.source_formula_weight, 0) * CASE
                WHEN
                    ac.fk_shipment_scheme IN (1 , 2, 3, 4)
                THEN
                    CASE
                    ...
                    END
                ELSE 0
            END / IF(seller_type = 'supplier', 1.1, 1) 'source_shipping_surcharge',
            (ac.shipping_amount / IF(seller_type = 'supplier', 1.1, 1)) + (IFNULL(rcc.shipment_cost_rate, 0) * IFNULL(IF(ac.is_source_package_weight, source_package_weight, source_dim_weight) / ac.source_formula_weight, 0) * CASE
                WHEN
                    ac.fk_shipment_scheme IN (1 , 2, 3, 4)
                THEN
                    CASE
                    ...
                    END
                ELSE 0
            END / IF(seller_type = 'supplier', 1.1, 1)) + total_shipment_fee_mp_seller - total_delivery_cost - ac.total_failed_delivery 'gain_loss_source'
    FROM
        (SELECT 
        *,
            (SELECT 
                ...
                FROM
                    scgl.anondb_extract
                WHERE
                    order_nr = ac.order_nr
                        AND id_package_dispatching = ac.id_package_dispatching
                        AND bob_id_supplier = ac.bob_id_supplier
                        AND ((delivered_date >= @extractstart
                        AND delivered_date < @extractend)
                        OR (not_delivered_date >= @extractstart
                        AND not_delivered_date < @extractend))) 'source_dim_weight',
            (SELECT 
                    SUM(IFNULL(CASE
                            WHEN
                                simple_weight > 0
                ...
                WHERE
                    order_nr = ac.order_nr
                        ...
                        AND delivered_date < @extractend)
                        OR (not_delivered_date >= @extractstart
                        AND not_delivered_date < @extractend))) 'source_package_weight',
            (SELECT 
                    GREATEST(SUM(IFNULL(CASE
                            WHEN
                                simple_weight > 0
                                    OR (simple_length * simple_width * simple_height) > 0
                            THEN
                ...
                FROM
                    scgl.anondb_extract
                WHERE
                    id_package_dispatching = ac.id_package_dispatching
                        AND ((delivered_date >= @extractstart
                        AND delivered_date < @extractend)
                        OR (not_delivered_date >= @extractstart
                        AND not_delivered_date < @extractend))) 'source_formula_weight',
            (SELECT 
                    CASE
                            WHEN
                                SUM(IFNULL(CASE
                ...
                WHERE
                    id_package_dispatching = ac.id_package_dispatching
                        ...
                        AND delivered_date < @extractend)
                        OR (not_delivered_date >= @extractstart
                        AND not_delivered_date < @extractend))) 'is_source_package_weight'
    FROM
        (SELECT 
        *,
            IF(fk_shipment_scheme IN (2 , 3, 4)
                AND first_shipped_date IS NOT NULL
                AND not_delivered_date IS NOT NULL, total_shipment_cost, 0) 'total_failed_delivery',
            CASE
            ...
            END 'gain_loss',
            CASE
                WHEN FIND_IN_SET(id_district, @jakarta) THEN 4
                WHEN FIND_IN_SET(id_district, @bodetabek) THEN 4
                ELSE fk_zone_type
            END 'fk_zone_type_temp',
            CASE
    ...
    LEFT JOIN scgl.rate_card_customer rcc ON ac.id_district = rcc.id_district
        AND ac.origin = rcc.origin) result
GROUP BY id_city