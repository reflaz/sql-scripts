...
SELECT 
    weight_bucket,
    SUM(count_pck) 'count_pck',
    SUM(count_qty) 'count_soi'
FROM
    (SELECT 
        *,
            CASE
                WHEN source_rounded_weight <= 0.17 THEN '<= 0.17 Kg'
                WHEN source_rounded_weight = 1 THEN '1 Kg'
                WHEN source_rounded_weight = 2 THEN '2 Kg'
                WHEN source_rounded_weight = 3 THEN '3 Kg'
                WHEN source_rounded_weight = 4 THEN '4 Kg'
                WHEN source_rounded_weight = 5 THEN '5 Kg'
                WHEN source_rounded_weight = 6 THEN '6 Kg'
                WHEN source_rounded_weight = 7 THEN '7 Kg'
                WHEN source_rounded_weight = 8 THEN '8 Kg'
                WHEN source_rounded_weight = 9 THEN '9 Kg'
                WHEN source_rounded_weight = 10 THEN '10 Kg'
                ELSE 'Above 10 Kg'
            END 'weight_bucket'
    FROM
        (SELECT 
        *,
            ...
            COUNT(DISTINCT bob_id_sales_order_item) 'count_qty',
            CEIL(GREATEST(SUM(IFNULL(CASE
                WHEN
                    simple_weight > 0
    ...
    FROM
        ...
        ae.*,
            ...
            CASE
                WHEN
                    (ac.order_date < '2017-01-01'
                        OR ac.first_shipped_date < '2017-01-01')
                        AND (ac.rounded_weight / ac.qty) > 7
                        AND ac.shipping_fee_to_customer = 0
                THEN
                    0
                WHEN
                    (ac.order_date >= '2017-01-01'
                        OR ac.first_shipped_date >= '2017-01-01')
                        AND (ac.rounded_weight / ac.qty) > 2
                        AND ac.shipping_fee_to_customer = 0
                THEN
                    0
                WHEN
                    ac.fk_shipment_scheme <> 5
                        AND ac.rounded_weight > 0
                        AND ac.total_shipment_cost = 0
                THEN
                    0
                WHEN ac.rounded_weight / ac.qty > 400 THEN 0
                ELSE 1
            END 'pass'
    FROM
        scgl.anondb_calculate ac
    LEFT JOIN scgl.anondb_extract ae ON ac.id_package_dispatching = ae.id_package_dispatching
        AND ac.order_nr = ae.order_nr
    ...
    WHERE
        ((ac.delivered_date >= @extractstart
            ...
            OR ac.first_shipped_date >= '2017-01-01')
            AND ac.fk_zone_type IN (3 , 4)
    HAVING pass = 1) ac
    GROUP BY bob_id_sales_order_item) ac) ac
