...
FROM
    (SELECT 
        order_value,
            ...
            SUM(total_paid_price) / COUNT(order_nr) 'aov',
            SUM(total_qty) / COUNT(order_nr) 'average_item_per_order',
            SUM(total_3pl_cost) 'total_3pl_cost',
            SUM(total_seller_charge) 'total_seller_charge',
            SUM(total_shipping_amount) 'total_shipping_amount',
            SUM(total_shipping_surcharge) 'total_shipping_surcharge',
            SUM(total_shipping_fee_to_customer) 'total_shipping_fee_to_customer',
            SUM(total_gain_loss) 'total_gain_loss'
    FROM
        (SELECT 
        *,
            ...
            CASE
                WHEN SUM(unit_price) < 30000 THEN 'Below 30k'
                WHEN SUM(unit_price) < 50000 THEN '30k - 50k'
                WHEN SUM(unit_price) < 100000 THEN '50k - 100k'
                ELSE 'Above 100k'
            END 'order_value'
    FROM
        (SELECT 
        *,
            ...
            CASE
                ...
                WHEN fk_shipment_scheme = 1 THEN 'DB'
                WHEN
                    tax_class = 'local'
                        AND fk_campaign IN (2 , 4)
                THEN
                    'MP Local - VIP'
                WHEN tax_class = 'local' THEN 'MP Local - Non VIP'
            END 'bu',
            CASE
    ...
    HAVING pass = 1) ac
    GROUP BY order_nr) ac
    GROUP BY order_value , fk_zone_type_temp) result