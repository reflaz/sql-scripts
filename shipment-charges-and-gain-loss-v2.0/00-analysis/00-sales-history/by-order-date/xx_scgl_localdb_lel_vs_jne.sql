...
SELECT 
    ...
    SUM(nmv) 'nmv',
    SUM(total_shipment_fee_mp_seller) 'shipment_fee_mp_seller',
    SUM(total_pickup_cost) 'pickup_cost',
    SUM(total_delivery_cost) 'delivery_cost_lel',
    SUM(delivery_cost_jne) 'delivery_cost_jne'
FROM
    (SELECT 
        ac.*,
            CASE
            ...
            IFNULL(total_shipment_fee_mp_seller, 0) + IFNULL(shipping_fee_to_customer, 0) - IFNULL(total_delivery_cost, 0) 'gain_loss_lel',
            IFNULL((rc.shipment_cost_rate * 0.7 * ac.rounded_weight) + total_insurance_charge, 0) 'delivery_cost_jne',
            IFNULL(total_shipment_fee_mp_seller, 0) + IFNULL(shipping_fee_to_customer, 0) - IFNULL((rc.shipment_cost_rate * 0.7 * ac.rounded_weight) + total_insurance_charge, 0) 'gain_loss_jne',
            CASE
