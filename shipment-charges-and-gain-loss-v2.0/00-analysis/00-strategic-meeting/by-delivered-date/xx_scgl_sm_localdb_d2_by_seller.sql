...
FROM
    (SELECT 
        ac.bob_id_supplier 'Seller ID',
            ...
            CASE
                WHEN MAX(IF(fk_campaign IN (2 , 4), 2, NULL)) THEN 'VIP'
                ELSE ''
            ...
            'Shipping fee credit received - NULL',
            'Shipping Reimbursement Received - NULL',
            'Payment charges paid - NULL',
            'FBL charges paid - NULL',
            'Penalties / service fees - NULL',
            'Marketing service fees - NULL',
            'Commission waivers - NULL',
            ...
            SUM(IFNULL(ac.paid_price / 1.1, 0) + IFNULL(ac.shipping_surcharge / 1.1, 0) + IFNULL(ac.shipping_amount / 1.1, 0) + IF(ac.coupon_type <> 'coupon', IFNULL(ac.coupon_money_value / 1.1, 0), 0)) 'NMV January (IDR)',
