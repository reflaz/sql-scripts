...
SELECT 
    ac.level1 'Category Name',
    SUM(IFNULL(ac.unit_price, 0)) 'Revenue Nov (based on unit price)',
    'Revenue Jan (based on unit price)',
    COUNT(ac.bob_id_sales_order_item) 'Items sold Jan',
    COUNT(DISTINCT ac.order_nr) 'Orders Jan',
    ...
    SUM(IFNULL(ac.total_shipment_fee_mp_seller, 0)) 'Delivery charges paid - Total',
    'FBL charges paid - NULL',
    'Penalties / service fees - NULL',
    'Marketing service fees - NULL',
    SUM(IFNULL(commission_should_be, 0)) 'Commission waivers',
    'Pricing subsidies - NULL',
    SUM(IFNULL(total_shipment_fee_mp_seller, 0) + IFNULL(shipping_fee_to_customer, 0) - IFNULL(total_shipment_cost, 0) - IFNULL(total_insurance_charge, 0)) 'Shipping Fee subsidies',
    SUM(IF(coupon_type = 'coupon',
        coupon_money_value,
        0)) 'Voucher amount (paid by Lazada) (IDR)',
    'Seller\'s Take Home Pay - NULL',
    'Seller\'s Take Home Rate - NULL',
    'Voucher+Commission waiver + Subsidies (princing+shipping) ($) - NULL',
    'Commission waiver + Subsidies (princing+shipping) ($) - NULL',
    SUM(IFNULL(ac.paid_price / 1.1, 0) + IFNULL(ac.shipping_surcharge / 1.1, 0) + IFNULL(ac.shipping_amount / 1.1, 0) + IF(ac.coupon_type <> 'coupon',
        IFNULL(ac.coupon_money_value / 1.1, 0),
        0)) 'NMV Jan (IDR)',
    SUM(IFNULL(ac.total_pickup_cost, 0)) 'Shipping costs (Pickup Cost Only)',
    SUM(CASE
    ...
    'PC2 (IDR) - NULL',
    'PC2 (%) - NULL',
    SUM(IFNULL(rounded_weight, 0)) / COUNT(bob_id_sales_order_item) 'Average Weight'
FROM
    (SELECT 
        ac.*,
            ct.level1_id,
            ct.level1,
            IFNULL(gc.general_commission, rc.general_commission) * unit_price / 100 'commission_should_be',
            CASE
                WHEN
    ...
    LEFT JOIN scgl.category_tree ct ON ac.primary_category = ct.lookup_cat_id
    LEFT JOIN zpcr.general_commission gc ON ac.primary_category = gc.lookup_cat_id
        AND ac.tax_class = gc.tax_class
    LEFT JOIN zpcr.general_commission rc ON ac.tax_class = rc.tax_class
        AND rc.lookup_cat_id = 1
    WHERE
        ac.delivered_date >= @extractstart
            AND ac.delivered_date < @extractend
            AND (ac.seller_type = 'supplier'
            OR ac.tax_class IN ('local'))
    HAVING pass = 1) ac
GROUP BY ac.level1_id;