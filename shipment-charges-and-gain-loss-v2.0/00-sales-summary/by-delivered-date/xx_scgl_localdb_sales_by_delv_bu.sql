/*-----------------------------------------------------------------------------------------------------------------------------------
-------------------------------------------------------------------------------------------------------------------------------------
Shipping Charges and Gain/Loss Summary by Delivered Date
...
SET @extractend = '2017-01-09';-- This MUST be D + 1
...
FROM
    (SELECT 
        CONCAT(IFNULL(ac.fk_shipment_scheme, 'xx'), IFNULL(ac.fk_zone_type_temp, 'xx'), IFNULL(ac.weight_bucket, 'xx')) formula,
            ac.shipment_scheme,
            CASE
                WHEN
            ...
            SUM(ac.qty) 'count_soi',
            SUM(ac.total_shipment_fee_mp_seller + ac.shipping_fee_to_customer - ac.total_delivery_cost) 'gain_loss'
    FROM
        (SELECT 
        paid_price,
            order_nr,
            ...
            CASE
                WHEN
                    fk_shipment_scheme = 7
                THEN
                    CASE
                        WHEN seller_type = 'supplier' THEN 3
                        WHEN sc_fee_2 > 0 THEN 1
                        ELSE 2
                    END
                ELSE fk_shipment_scheme
            END 'fk_shipment_scheme',
            CASE
                WHEN fk_shipment_scheme = 7 THEN 0
    ...
    HAVING pass = 1) ac
    GROUP BY fk_shipment_scheme , fk_zone_type_temp , weight_bucket) result