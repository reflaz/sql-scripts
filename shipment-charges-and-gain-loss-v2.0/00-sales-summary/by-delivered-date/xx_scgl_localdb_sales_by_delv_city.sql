...
SELECT 
    CONCAT(IFNULL(fz.id_city, 'xx'),
            IFNULL(ac.fk_zone_type_temp, 'xx'),
            IFNULL(ac.weight_bucket, 'xx')) formula,
    fz.region,
    fz.city,
    CASE
        WHEN
...
FROM
    ...
    HAVING pass = 1) ac
        LEFT JOIN
    scgl.free_zone fz ON ac.id_district = fz.id_district
        AND ac.active = fz.active
GROUP BY fz.id_city , fk_zone_type_temp , weight_bucket