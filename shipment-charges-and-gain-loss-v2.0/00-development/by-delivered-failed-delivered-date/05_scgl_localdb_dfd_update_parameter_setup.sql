...
WHERE
    ((ac.delivered_date >= @extractstart
        ...
        OR (ac.not_delivered_date >= @extractstart
...
WHERE
    ((ce.delivered_date >= @extractstart
        AND ce.delivered_date < @extractend)
        OR (ce.not_delivered_date >= @extractstart
        AND ce.not_delivered_date < @extractend))
        AND ce.seller_type = 'merchant'
...
UPDATE anondb_calculate ce 
SET 
    ce.shipment_fee_mp_seller_rate = CASE
        WHEN
            fk_campaign = 2
        THEN
            CASE
                WHEN fk_shipment_scheme = 4 THEN 3000
                ELSE 4000
            END
        WHEN
            fk_shipment_scheme = 4
        THEN
            CASE
                WHEN pickup_provider_type = 'LEX' THEN 6000
                ELSE 6500
            END
        WHEN pickup_provider_type = 'LEX' THEN 6464
        ELSE 7000
    END
WHERE
    ((ce.delivered_date >= @extractstart
        ...
        OR (ce.not_delivered_date >= @extractstart
        AND ce.not_delivered_date < @extractend))
        AND GREATEST(ce.order_date,
            IFNULL(ce.first_shipped_date, 1)) >= '2017-07-01'
        AND ce.fk_shipment_scheme IN (2 , 4, 7);
