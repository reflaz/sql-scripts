...
UPDATE scgl.anondb_calculate ac
            ...
            CASE
                WHEN
                    fk_shipment_scheme = 1
                        -- OR order_date >= '2017-04-01'
                        -- OR first_shipped_date >= '2017-04-01'
                        -- OR delivered_date >= '2017-04-01'
                        -- OR not_delivered_date >= '2017-04-01'
                THEN
                    CASE
    ...
    WHERE
        FIND_IN_SET(order_nr, @order_nr)) ae) ae
...
FROM
    ...
    LEFT JOIN scgl.anondb_calculate ac ON ae.order_nr = ac.order_nr
        ...
        AND ae.bob_id_supplier = ac.bob_id_supplier
    WHERE
        FIND_IN_SET(ae.order_nr, @order_nr)
            AND ac.bob_id_sales_order_item IS NULL) ae) ae
    GROUP BY ae.order_nr , ae.id_package_dispatching , ae.bob_id_supplier) ae;
