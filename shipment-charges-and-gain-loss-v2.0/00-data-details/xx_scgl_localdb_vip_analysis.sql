...
SELECT 
    ...
    SUM(shipping_fee_to_customer) 'total_shipping_fee_from_customer',
    SUM(coupon) 'total_coupon',
    SUM(credit) 'total_credit',
    SUM(cart_rule_discount) 'total_cart_rule_discount',
    COUNT(DISTINCT order_nr) 'total_so',
    SUM(qty) 'total_soi',
    SUM(rounded_weight) / SUM(qty) 'average_weight',
    SUM(shipment_fee_mp_seller) 'shipment_fee_mp_seller',
    ...
    SUM(seller_shipping_subsidy) 'seller_shipping_subsidy',
    SUM(customer_shipping_subsidy) 'customer_shipping_subsidy',
    SUM(commission_subsidy) 'commission_subsidy'
FROM
    (SELECT 
        ac.bob_id_sales_order_item,
            ...
            IF(ac.coupon_type = 'coupon', ac.coupon_money_value, 0) 'coupon',
            IF(ac.coupon_type <> 'coupon', ac.coupon_money_value, 0) 'credit',
            ac.cart_rule_discount,
            ac.last_status,
            ac.order_date,
            ac.first_shipped_date,
            ac.last_shipped_date,
            ac.delivered_date,
            ac.not_delivered_date,
            ac.closed_date,
            ac.refund_completed_date,
            ac.package_number,
            ...
            CASE
                ...
                ELSE ac.zone_type
            END 'zone_type',
            ac.shipment_fee_mp_seller_rate,
            ac.shipment_fee_mp_seller,
            ac.total_insurance_fee,
            ac.total_shipment_fee_mp_seller,
            ac.shipment_cost_rate,
            ac.shipment_cost_discount,
            ac.total_shipment_cost,
            ac.pickup_cost,
            ac.pickup_cost_discount,
            ...
            ac.total_shipment_fee_mp_seller - (CASE
                WHEN
                ...
                THEN
                    6464
                ELSE 7000
            END * ac.rounded_chargeable_weight) - ac.total_insurance_charge 'seller_shipping_subsidy',
            ac.shipping_fee_to_customer - ac.total_delivery_cost + (CASE
                WHEN
                    ac.fk_shipment_scheme = 2
                        AND ac.pickup_provider_type = 'LEX'
                THEN
                    6464
                ELSE 7000
            END * ac.rounded_chargeable_weight) + ac.total_insurance_charge 'customer_shipping_subsidy',
            IFNULL(ac.marketplace_commission_fee, 0) - SUM(IFNULL(IFNULL(gc.general_commission, rc.general_commission), 0) * IFNULL(ae.unit_price, 0) / 100 * 1.1) 'commission_subsidy'
    FROM
        scgl.campaign_tracker ct
    LEFT JOIN scgl.anondb_calculate ac ON ct.bob_id_supplier = ac.bob_id_supplier
    LEFT JOIN scgl.anondb_extract ae ON ac.order_nr = ae.order_nr
        AND ac.id_package_dispatching = ae.id_package_dispatching
        AND ac.bob_id_supplier = ae.bob_id_supplier
    LEFT JOIN zpcr.general_commission gc ON ae.primary_category = gc.lookup_cat_id
        AND ae.tax_class = gc.tax_class
    LEFT JOIN zpcr.general_commission rc ON ae.tax_class = rc.tax_class
        AND rc.lookup_cat_id = 1
    WHERE
        ct.end_date > NOW()
            AND ct.fk_campaign IN (2)
            AND ac.fk_shipment_scheme IN (1 , 2, 3, 4, 7)
            AND ac.rounded_weight <= 400
    GROUP BY ac.order_nr , ac.id_package_dispatching , ac.bob_id_supplier) result
