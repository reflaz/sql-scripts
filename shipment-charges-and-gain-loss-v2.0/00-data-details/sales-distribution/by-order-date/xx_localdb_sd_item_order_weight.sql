...
FROM
    (SELECT 
        price_bucket,
			city,
            zone_type,
            COUNT(IF(weight_bucket = '<= 1 Kg', bob_id_sales_order_item, NULL)) 'count_soi_<=1Kg',
            COUNT(IF(weight_bucket = '<= 1.3 Kg', bob_id_sales_order_item, NULL)) 'count_soi_<=1.3Kg',
            COUNT(IF(weight_bucket = '<= 2 Kg', bob_id_sales_order_item, NULL)) 'count_soi_<=2Kg',
            COUNT(IF(weight_bucket = '<= 2.3 Kg', bob_id_sales_order_item, NULL)) 'count_soi_<=2.3Kg',
            COUNT(IF(weight_bucket = '> 2.3 Kg', bob_id_sales_order_item, NULL)) 'count_soi_>2.3Kg',
    ...
    FROM
        ...
        bob_id_sales_order_item,
            ...
            END 'weight_bucket',
            CASE
                WHEN IFNULL(unit_price, 0) <= 30000 THEN '<= 30k'
                WHEN IFNULL(unit_price, 0) <= 50000 THEN '<= 50k'
                WHEN IFNULL(unit_price, 0) <= 75000 THEN '<= 75k'
                WHEN IFNULL(unit_price, 0) <= 100000 THEN '<= 100k'
                WHEN IFNULL(unit_price, 0) <= 150000 THEN '<= 150k'
                ELSE '> 150k'
            END 'price_bucket',
            fz.city
    FROM
