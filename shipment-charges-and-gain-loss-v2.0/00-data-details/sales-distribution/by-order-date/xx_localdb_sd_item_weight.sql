...
SELECT 
    zone_type,
    SUM(IF(multiple_origin = 0, qty, 0)) 'count_soi_single',
    COUNT(DISTINCT IF(multiple_origin = 0, order_nr, NULL)) 'count_so_single',
    SUM(IF(multiple_origin = 0, total_unit_price, 0)) 'total_unit_price_single',
    
    SUM(IF(multiple_origin = 1, qty, 0)) 'count_soi_multiple',
    COUNT(DISTINCT IF(multiple_origin = 1, order_nr, NULL)) 'count_so_multiple',
    SUM(IF(multiple_origin = 1, total_unit_price, 0)) 'total_unit_price_multiple',
    
    SUM(qty) 'count_soi',
    COUNT(DISTINCT order_nr) 'count_so',
    SUM(total_unit_price) 'total_unit_price'
FROM
    (SELECT 
        ac.*,
            ...
            CASE
                WHEN COUNT(DISTINCT origin) > 1 THEN 1
                ELSE 0
            END 'multiple_origin',
            CASE
                WHEN FIND_IN_SET(ac.id_district, @jakarta) THEN 4
                WHEN FIND_IN_SET(ac.id_district, @bodetabek) THEN 4
                ELSE fz.fk_zone_type
            END 'fk_zone_type',
            CASE
                WHEN FIND_IN_SET(ac.id_district, @jakarta) THEN 'Jabodetabek'
                WHEN FIND_IN_SET(ac.id_district, @bodetabek) THEN 'Jabodetabek'
                ELSE zt.zone_type
            END 'zone_type'
    FROM
        (SELECT 
    ...
    HAVING formula_weight_temp < 400
        AND fk_shipment_scheme IN (1 , 2, 3, 4, 7)) ac
    ...
    GROUP BY order_nr
    HAVING formula_weight > 2) ac
GROUP BY fk_zone_type