...
FROM
    (SELECT 
        price_bucket,
            city,
            zone_type,
            SUM(IF(weight_bucket = '<= 1 Kg', count_soi, 0)) 'count_soi_<=1Kg',
            SUM(IF(weight_bucket = '<= 1.3 Kg', count_soi, 0)) 'count_soi_<=1.3Kg',
            SUM(IF(weight_bucket = '<= 2 Kg', count_soi, 0)) 'count_soi_<=2Kg',
            SUM(IF(weight_bucket = '<= 2.3 Kg', count_soi, 0)) 'count_soi_<=2.3Kg',
            SUM(IF(weight_bucket = '> 2.3 Kg', count_soi, 0)) 'count_soi_>2.3Kg',
            SUM(IF(weight_bucket = '<= 1 Kg', paid_price, 0)) 'paid_price_<=1Kg',
            SUM(IF(weight_bucket = '<= 1.3 Kg', paid_price, 0)) 'paid_price_<=1.3Kg',
            SUM(IF(weight_bucket = '<= 2 Kg', paid_price, 0)) 'paid_price_<=2Kg',
            SUM(IF(weight_bucket = '<= 2.3 Kg', paid_price, 0)) 'paid_price_<=2.3Kg',
            SUM(IF(weight_bucket = '> 2.3 Kg', paid_price, 0)) 'paid_price_>2.3Kg'
    FROM
        (SELECT 
        bob_id_sales_order_item,
            order_nr,
            IFNULL(paid_price_temp, 0) 'paid_price',
            IFNULL(unit_price_temp, 0) 'unit_price',
            IFNULL(formula_weight, 0) 'formula_weight',
            CASE
                WHEN FIND_IN_SET(ae.id_district, @jakarta) THEN 'Jabodetabek'
                WHEN FIND_IN_SET(ae.id_district, @bodetabek) THEN 'Jabodetabek'
                ELSE zt.zone_type
            END 'zone_type',
            CASE
                WHEN IFNULL(formula_weight, 0) <= 1 THEN '<= 1 Kg'
                WHEN IFNULL(formula_weight, 0) <= 1.3 THEN '<= 1.3 Kg'
                WHEN IFNULL(formula_weight, 0) <= 2 THEN '<= 2 Kg'
                WHEN IFNULL(formula_weight, 0) <= 2.3 THEN '<= 2.3 Kg'
                ELSE '> 2.3 Kg'
            END 'weight_bucket',
            CASE
                WHEN IFNULL(unit_price_temp, 0) <= 30000 THEN '<= 30k'
                WHEN IFNULL(unit_price_temp, 0) <= 50000 THEN '<= 50k'
                ELSE '> 50k'
            END 'price_bucket',
            fz.city,
            count_soi_temp 'count_soi',
            count_so_temp 'count_so'
    FROM
        (SELECT 
        *,
            SUM(IFNULL(unit_price, 0)) 'unit_price_temp',
            SUM(IFNULL(paid_price, 0)) 'paid_price_temp',
            COUNT(bob_id_sales_order_item) 'count_soi_temp',
            COUNT(DISTINCT order_nr) 'count_so_temp',
    ...
    GROUP BY zone_type , price_bucket) ae