...
FROM
    (SELECT 
        p.sku AS sku,
            ...
            CASE
                WHEN i.fk_origin_warehouse = 1 THEN 'Default ID Warehouse'
                WHEN i.fk_origin_warehouse = 2 THEN 'Surabaya'
                WHEN i.fk_origin_warehouse = 3 THEN 'Medan'
                WHEN i.fk_origin_warehouse = 4 THEN 'TImesXBWH'
            END AS warehouse_origin,
            CASE
                WHEN i.fk_current_warehouse = 1 THEN 'Default ID Warehouse'
                WHEN i.fk_current_warehouse = 2 THEN 'Surabaya'
                WHEN i.fk_current_warehouse = 3 THEN 'Medan'
                WHEN i.fk_current_warehouse = 4 THEN 'TImesXBWH'
            END AS current_warehouse,
            ship.shipment_provider_name,
            pa.package_number laz_pck_number,
            ih.history_created_at AS rts_date,
            ih2.history_created_at AS shipped_date,
    ...
    FROM
        (SELECT 
        MAX(id_inventory_history) 'id_inventory_history'
    FROM
        oms_live.wms_inventory_history
    WHERE
        history_created_at >= @extractstart
            AND history_created_at < @extractend) pop
    LEFT JOIN oms_live.wms_inventory_history i ON pop.id_inventory_history = i.id_inventory_history
    LEFT JOIN oms_live.wms_inventory_history ih ON i.id_inventory = ih.id_inventory
        AND ih.id_inventory_history = (SELECT 
            MIN(id_inventory_history)
        FROM
            oms_live.wms_inventory_history
        WHERE
            uid = ih.uid
    ...
    LEFT JOIN oms_live.wms_inventory_history ih2 ON ih.uid = ih2.uid
        AND ih2.id_inventory_history = (SELECT 
            MIN(id_inventory_history)
        FROM
            oms_live.wms_inventory_history
        WHERE
            uid = ih.uid AND fk_inventory_status = 5)
    LEFT JOIN oms_live.wms_inventory_history ih3 ON ih.uid = ih3.uid
        AND ih3.id_inventory_history = (SELECT 
            MIN(id_inventory_history)
        FROM
            oms_live.wms_inventory_history
        WHERE
            uid = ih.uid AND fk_inventory_status = 6)
    ...
    LEFT JOIN bob_live.sales_order_item bob_soi ON bob_soi.id_sales_order_item = soi.bob_id_sales_order_item
    LEFT JOIN bob_live.catalog_simple cs ON bob_soi.sku = cs.sku
