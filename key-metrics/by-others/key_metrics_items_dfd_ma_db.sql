...
FROM
    (SELECT 
        package_status,
            COUNT(IF(seller_type = 'MA', bob_id_sales_order_item, NULL)) 'count_soi_ma',
            COUNT(IF(seller_type = 'DB', bob_id_sales_order_item, NULL)) 'count_soi_db'
    FROM
        (SELECT 
        psh.fk_package_status,
            psh.package_status,
            ...
            CASE
                ...
                WHEN
                    shipment_provider_name = 'Digital Delivery'
                        OR shipment_provider_name = 'Digital Delivery'
                THEN
                    'DIGITAL'
    ...
    HAVING seller_type IN ('MA' , 'DB')) result
    GROUP BY fk_package_status) result