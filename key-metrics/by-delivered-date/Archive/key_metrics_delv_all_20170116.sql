...
/* Table A - NMV Summary by Zone Type */
...
/* Table B - NMV Summary by Seller Type */
...
/* Table C - Summary by BU and Category Lv 1 */
...
FROM
    ...
    GROUP BY id_sales_order_item) result
    GROUP BY lvl1_id) result;

/* Table D - Summary by Category Lv 1 Retail*/
...
/* Table E - Summary by Seller Type (incl. FBL) */
    ...
    (SELECT 
        so.id_sales_order,
            ...
            CASE
                ...
                THEN
                    'CB'
                WHEN soi.fk_shipping_type = 3 THEN 'FBL'
                ELSE 'NON-FBL'
            END 'seller_type'
    FROM
        (SELECT 
...
/* Table G - Coupon and Credit Summary */

SELECT 
    seller_type,
    SUM(coupon) 'coupon',
    SUM(credit) 'credit',
    SUM(cart_rule_discount) 'cart_rule'
FROM
    (SELECT 
        so.id_sales_order,
            soi.bob_id_sales_order_item,
            ...
            CASE
                ...
                ELSE 'NON-VIP'
            END 'seller_type',
            IF(so.fk_voucher_type = 3, IFNULL(soi.coupon_money_value / 1.1, 0), 0) 'coupon',
            IF(so.fk_voucher_type <> 3, IFNULL(soi.coupon_money_value / 1.1, 0), 0) 'credit'
    FROM
        (SELECT 
        fk_package
    FROM
...
/* Table H - Paymanet Method NMV Summary */
