...
SELECT 
    *,
    total_paid_by_customer-gross_amount 'oustanding_amount'
FROM
    (SELECT 
        order_nr,
            payment_method,
...
WHERE
    abs(total_paid_by_customer - gross_amount) > 10000
        or incoming_order_nr IS NULL;

truncate table non_cod_recon.ar_outstanding;

insert into non_cod_recon.ar_outstanding
SELECT 
    pop.*
FROM
    (SELECT 
