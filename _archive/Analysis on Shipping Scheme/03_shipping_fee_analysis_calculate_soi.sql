...
FROM
    ...
    FROM
        (SELECT 
        *,
            IFNULL(shipping_surcharge, 0) 'total_shipping_surcharge',
            IFNULL(unit_price, 0) 'total_unit_price',
            IFNULL(paid_price, 0) 'total_paid_price',
            IFNULL(coupon_money_value, 0) 'total_coupon_money_value',
            IFNULL(cart_rule_discount, 0) 'total_cart_rule_discount',
            IFNULL(weight, 0) 'total_weight',
            IFNULL(volumetric_weight, 0) 'total_volumetric_weight',
            IF(IFNULL(weight, 0) > IFNULL(volumetric_weight, 0), IFNULL(weight, 0), IFNULL(volumetric_weight, 0)) 'formula_weight'
    FROM
        oms_data
    -- WHERE
