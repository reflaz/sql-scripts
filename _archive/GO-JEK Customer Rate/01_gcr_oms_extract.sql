/*-----------------------------------------------------------------------------------------------------------------------------------
-------------------------------------------------------------------------------------------------------------------------------------
GO-JEK Customer Rate OMS Extract
...
SET @extractend = '2016-09-01'; -- This MUST be H + 1
SET @district = '6653,6652,6651,6650,6649,6648,6647,6646,6644,6643,6642,6641,6640,6639,6638,6637,6635,6634,6633,6632,6631,6630,6629,6628,6627,6626,6624,6623,6622,6621,6620,6619,6618,6617,6616,6615,6613,6612,6611,6610,6609,6608';
...
FROM
    ...
    LEFT JOIN screport.transaction tr ON scsoi.id_sales_order_item = tr.ref
        AND tr.fk_transaction_type IN (7)
    WHERE
    ...
    WHERE
        (sup.type = 'supplier'
            OR sel.short_code IN ('ID10118' , 'ID10725', 'ID116S2', 'ID100N7', 'ID1022U', 'ID101HG', 'ID102P3', 'ID102GY', 'ID101EP', 'ID102T9', 'ID100X8', 'ID1028C', 'ID10726', 'ID101SU'))
            AND FIND_IN_SET(dst.id_customer_address_region, @district)
    GROUP BY soi.id_sales_order_item) result) result
    HAVING item_rounded_weight <= 7) result