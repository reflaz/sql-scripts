/*-----------------------------------------------------------------------------------------------------------------------------------
-------------------------------------------------------------------------------------------------------------------------------------
FBL Shipping OMS Extract by Package Number
 
Prepared by		: R Maliangkay
Modified by		: RM
Version			: 1.1
Changes made	: - Add first shipment provider

Instructions	: - Go to your excel file
				  - Format the parameter in excel using this formula: ="'"&Column&"'," --> change Column accordingly
				  - Insert formatted Package Number in >> WHERE tracking_number IN () << part of the script
...
FROM
    (SELECT 
        so.order_nr 'order_nr',
            ...
            sp.shipment_provider_name 'first_shipment_provider',
    ...
    WHERE
        pck.package_number IN ('')) result
    LEFT JOIN oms_live.oms_package_dispatching_history pdh ON result.id_package = pdh.fk_package
