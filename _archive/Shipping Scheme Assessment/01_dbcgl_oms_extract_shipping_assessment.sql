...
FROM
    ...
    WHERE
        sup.type = 'supplier'
            ...
            AND sp1.shipment_provider_name NOT LIKE '%LEX%'
            AND sp2.shipment_provider_name NOT LIKE '%LEX%')
    GROUP BY soi.id_sales_order_item
    HAVING first_shipment_provider IS NOT NULL
        AND last_shipment_provider IS NOT NULL) result) result) result