...
SELECT 
    ...
    total_charge_to_seller + total_charge_to_customer 'PC1 JNE',
    CASE
        WHEN zone = 'Free Zone' THEN ((7000 * rounded_weight_non_bulky) - insurance_from_3pl) * - 1
        WHEN zone = 'Paid Zone' THEN total_charge_from_3pl_jne
        ELSE 0
    END 'PC2 JNE',
    CASE
        WHEN zone = 'Free Zone' THEN total_charge_from_3pl_jne + (7000 * rounded_weight_non_bulky) - insurance_from_3pl
        ELSE 0
    END 'PC3 JNE',
    ...
    total_charge_to_seller + total_charge_to_customer 'PC1 LEL',
    CASE
        WHEN zone = 'Free Zone' THEN ((7000 * rounded_weight_non_bulky) - insurance_from_3pl) * - 1
        WHEN zone = 'Paid Zone' THEN total_charge_from_3pl_lel
        ELSE 0
    END 'PC2 LEL',
    CASE
        WHEN zone = 'Free Zone' THEN total_charge_from_3pl_lel + (7000 * rounded_weight_non_bulky) - insurance_from_3pl
        ELSE 0
    END 'PC3 LEL'
FROM
    (SELECT 
