...
FROM
    ...
    FROM
        ...
        result.*,
            soi.*,
            IFNULL(asc_soi.id_sales_order_item, lsc_soi.id_sales_order_item) 'sc_sales_order_item',
            SUM(lsc_tr.value) 'value'
    FROM
        (SELECT 
        pck.id_package,
            pck.package_number,
    ...
    LEFT JOIN oms_live.oms_shipment_provider sp2 ON pd.fk_shipment_provider = sp2.id_shipment_provider
        ...
        WHERE
            shipment_provider_name LIKE '%MP%'
                ...
                OR shipment_provider_name LIKE '%LEX%'
                OR shipment_provider_name LIKE '%NINJA VAN%')
    ...
    LEFT JOIN screport.sales_order_item lsc_soi ON soi.id_sales_order_item = lsc_soi.src_id
    LEFT JOIN screport.transaction lsc_tr ON lsc_soi.id_sales_order_item = lsc_tr.ref
        AND lsc_tr.fk_transaction_type = 7
