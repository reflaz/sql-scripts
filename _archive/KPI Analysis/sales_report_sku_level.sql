...
SELECT 
    sku, supplier_type, lvl1, total_paid_price, total_qty
FROM
    (SELECT 
        *, SUM(qty) 'total_qty', SUM(paid_price) 'total_paid_price'
    FROM
        (SELECT 
        bsoi.id_sales_order_item,
            bsoi.sku,
            bsoi.paid_price,
    ...
    GROUP BY bsoi.id_sales_order_item) result
    GROUP BY sku) result
        LEFT JOIN
    (SELECT 
