...
--        so.created_at >= '2015-01-01'  -- CONCAT(YEAR(NOW()), '-01-01')
--            AND so.created_at < '2015-07-01' 
--            AND so.payment_method = 'CashOnDelivery'
            ) so
    INNER JOIN oms_live.ims_sales_order_item soi ON so.id_sales_order = soi.fk_sales_order
        AND fk_sales_order_item_status IN ('5' , '8', '11', '27', '38', '42', '55', '56', '57', '68', '78')
    ...
    oms_live.oms_shipment_provider oms_sp3 ON oms_pd3.fk_shipment_provider = oms_sp3.id_shipment_provider
        LEFT JOIN
    bob_live.supplier bob_sup ON oms_so.bob_id_supplier = bob_sup.id_supplier
-- WHERE
--    bob_sup.type = 'supplier'
GROUP BY oms_pck.id_package;