...
FROM
    (SELECT 
        res.*,
            ...
            (IFNULL((SELECT 
                ...
                FROM
                    asc_live.transaction tr
                LEFT JOIN asc_live.transaction_archive ta ON ta.number = tr.number
                WHERE
                    tr.ref = sc_id_sales_order_item
    ...
    LEFT JOIN oms_live.oms_shipping_type st ON st.id_shipping_type
        AND soi.fk_shipping_type
    ...
    WHERE
        inv.uid IN ()
    GROUP BY bob_id_sales_order_item) res) result