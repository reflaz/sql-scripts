...
FROM
    (SELECT 
        res.*,
            ...
            (IFNULL((SELECT 
                ...
                WHERE
                    ta.ref = sc_id_sales_order_item
                        AND ta.fk_transaction_type IN (19 , 20, 36, 37, 64, 104, 138, 140, 141, 144, 145)), 0)) * - 1 'shipping_fee_adjustment'
    FROM
        (SELECT 
        soi.bob_id_sales_order_item,
            ...
            srs.name 'return_status',
            sris.name 'return_item_status'
    FROM
        oms_live.oms_supplier_return sr
    LEFT JOIN oms_live.oms_supplier_return_item sri ON sr.id_supplier_return = sri.fk_supplier_return
    LEFT JOIN oms_live.oms_supplier_return_item_status sris ON sri.fk_supplier_return_item_status = sris.id_supplier_return_item_status
    LEFT JOIN oms_live.oms_supplier_return_status srs ON sr.fk_supplier_return_status = srs.id_supplier_return_status
    LEFT JOIN oms_live.wms_inventory inv ON inv.id_inventory = sri.fk_inventory
    LEFT JOIN oms_live.ims_purchase_order_item poi ON inv.fk_purchase_order_item = poi.id_purchase_order_item
    LEFT JOIN oms_live.ims_sales_order_item soi ON soi.id_sales_order_item = poi.fk_sales_order_item
    ...
    LEFT JOIN oms_live.oms_package_item pi ON pi.fk_inventory = inv.id_inventory
    ...
    WHERE
        sr.return_number IN ()) res) result